...
import (
	...
	"github.com/alclssna33/codex_to_telegram/internal/controlapi"
...
func runWithIO(args []string, in io.Reader, out io.Writer) error {
	...
	case "secrets":
		return runSecrets(args[1:], in, out)
	case "service":
		return runService(args[1:], in, out)
	case "daemon":
		if len(args) > 2 || len(args) == 2 && args[1] != "run" {
...
func runStatus(cfg config.Config, out io.Writer) error {
	...
	lines := []string{
		...
		fmt.Sprintf("OpenAI configured: %t", strings.TrimSpace(cfg.OpenAIAPIKey) != ""),
...
func runDoctor(cfg config.Config, out io.Writer) error {
	...
	doctor["credential_status"] = map[string]bool{
		"telegram_configured": strings.TrimSpace(cfg.TelegramBotToken) != "",
		"openai_configured":   strings.TrimSpace(cfg.OpenAIAPIKey) != "",
	}
...
func runInit(args []string, in io.Reader, out io.Writer) error {
	...
	token, err := promptInitTelegramToken(reader, in, out)
	if err != nil {
		return err
	}
	...
	storedOutsideConfig, err := persistInitTelegramToken(token)
	if err != nil {
		return err
	}
...
func writeConfigEnv(path string, values map[string]string, force bool) error {
	...
	_, _ = fmt.Fprintln(file, "# Keep this file private. Windows credentials are stored separately.")
	for _, key := range keys {
