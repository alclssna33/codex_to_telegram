...
Local completion notifications for OpenAI Codex Desktop and CLI, delivered to
Telegram through a Go daemon. This public distribution is derived from
[`mideco-tech/codex-tg`](https://github.com/mideco-tech/codex-tg); attribution
and license notices are retained in [NOTICE](NOTICE) and [LICENSE](LICENSE).

The supported Windows installed profile is deliberately narrow:

- Run `CTR_GO_PROFILE=notifier`.
- Receive one Telegram notification when a local Codex Desktop or CLI
  conversation completes, fails, or is interrupted.
- Monitor user-facing conversations across all local working folders; a
  `projects.json` registry is not required.
- Keep Telegram read-only: it never starts, resumes, forks, steers, approves,
  denies, answers, or interrupts a Codex conversation.
- Use only `/start`, `/help`, `/status`, and `/repair` in Telegram.
- Preserve `minimal` only as an explicit rollback profile for the older
  registered-project interactive bridge.

For the Windows setup and recovery procedure, see
[docs/operations/windows-install.md](docs/operations/windows-install.md).

The broader command-oriented Telegram UI below is the legacy/minimal rollback
surface, not the installed Windows notifier contract.
...
- Use Telegram today as a high-signal completion notification surface, with the
  older interactive bridge kept as an explicit rollback path.
...
This repository currently focuses on the Windows notifier profile.
...
- Notifier profile: read-only global completion notifications over local Codex
  App Server `thread/list` and `thread/read`.
- Rollback interactive profiles: thread-first routing over local Codex App Server.
- Global observer for GUI/CLI runs, with polling fallback through `thread/read`.
...
In the Windows notifier profile, `/start` registers the owner DM as the
notification target. The daemon observes eligible CLI/Desktop threads across
local folders and sends one completion/failure notification with folder name,
optional title, and a bounded summary. Plain text, replies, voice notes,
attachments, and callbacks do not reach Codex.

### Minimal Rollback Profile

Set `CTR_GO_PROFILE=minimal` only when you deliberately need the older
registered-project interactive bridge. That rollback profile requires
`CTR_GO_PROJECTS_FILE`, exposes project/conversation pickers, can start or
continue Telegram-originated work, and follows the writer-ownership caveats in
ADR-020 and ADR-021.

### Windows Linked Task Handoff

Windows minimal profile에서 Telegram으로 Desktop 작업을 이어갈 때는 원본 작업을
직접 빼앗지 않습니다. 첫 Telegram 명령은 원본 완료 지점까지의 기록을 복사한
하나의 연결 작업을 만들고, 제목은 보통 `원본 제목 · 텔레그램 연동` 형태로
붙입니다.

Telegram 작업이 끝나면 bridge가 자기 writer를 놓습니다. 그 뒤 Codex Desktop에서
새로 생긴 같은 연결 작업을 열면 원본 기록과 Telegram 턴이 한 작업 안에 이어져
보입니다. 이후에는 원본 작업이 아니라 이 연결 작업에서 계속 작업하는 것이
정상 흐름입니다.

Codex Desktop이 연결 작업을 열어 둔 상태에서 Telegram 명령을 보내면 bridge는
새 fork를 만들거나 몰래 실행하지 않습니다. Codex에서 해당 연결 작업을 닫거나
다른 작업으로 이동한 뒤 다시 보내세요. 사용하는 Codex 빌드가 이동 후에도
writer를 잡고 있으면 Codex 앱을 종료한 뒤 같은 Telegram 명령을 다시 보내면
됩니다.

`/status`의 `State: bridge released`는 bridge가 놓았다는 뜻이지 Codex Desktop도
닫혔다는 보증은 아닙니다. 다음 Telegram 명령이 실제 소유권 확인 시도입니다.
`/repair`는 애매하게 남은 로컬 생성 상태와 제목 재시도 표시만 정리하며, 연결
작업을 resume하거나 새 모델 턴을 시작하지 않습니다.

Telegram 승인/거절 버튼은 bridge worker가 시작한 Telegram 턴에 대해서만
동작합니다. Codex Desktop에서 시작한 턴의 승인은 Telegram으로 알림이 오더라도
Codex Desktop 안에서 처리해야 합니다. 음성 메시지도 먼저 transcript 미리보기를
확인한 뒤 실행해야 하며, 실행 후에는 같은 연결 작업과 동일한 단일 writer 규칙을
따릅니다.
...
Telegram commands in the installed `notifier` profile:

- `/start`, `/help`, `/status`, `/repair`

Rollback `minimal`/legacy Telegram commands:
...
- `CTR_GO_PROFILE` (`notifier` for the installed Windows profile; `minimal` only for rollback)
- `CTR_GO_CODEX_BIN`
