...
Rollback `minimal` and legacy interactive profiles:
...
- The installed Windows profile is `notifier`; `minimal` is an explicit
  rollback profile.
- `/start` in the owner DM records or confirms the single notification target.
- `/help` lists only `/start`, `/help`, `/status`, and `/repair`.
- `/status` reports readiness, App Server polling state, observation counts,
  delivery backlog, sweep cursor, target state, and the last sanitized error.
- `/repair` restarts only the read-only App Server polling session and discovery
  path. It does not resume, fork, start, steer, interrupt, approve, deny,
  answer, archive, or otherwise mutate a Codex conversation.
- Discovery is global for interactive Codex CLI/Desktop sources and sends no
  `cwd` filter. Project registry membership is not consulted.
- The App Server operation set is handshake plus `thread/list` and
  `thread/read(includeTurns=true)`. Any writer method is outside the notifier
  contract.
- Notifications are terminal-only for `completed`, `failed`, and
  `interrupted` outcomes. Historical terminal threads at first activation are
  baselined without replay.
- Notification content includes only the final folder name, optional sanitized
  title, and bounded local summary or fixed fallback text. Full local paths,
  prompt text, final answer bodies, owner IDs, tokens, and raw App Server
  payloads are not logged or persisted in ordinary observation state.
- Plain text, replies, voice notes, attachments, and unsupported callbacks are
  ignored or answered as notifier-only management input and make zero Codex
  write calls. Voice input is not downloaded or transcribed in notifier mode.

### Interactive rollback profiles
...
## Direct And Linked Task Ownership Handoff

- `/start` existing-task selection binds the exact selected Codex task id.
- Selecting source task `S` stays bound to `S`, even if a historical linked row
  `S -> L` exists.
- A plain Telegram message after selecting source task `S` attempts
  `thread/resume(S)` and starts the turn on `S` when ownership is available.
- If Codex Desktop still owns selected source task `S`, the bridge fails closed
  with close-and-retry guidance. It creates no fork/link, starts no turn, queues
  no prompt, and persists no rejected plaintext prompt.
- One Telegram chat/topic plus one source Codex task may still have one
  canonical linked task for ADR-020 compatibility flows.
- The linked task title should be meaningful to the operator, normally `source title · 텔레그램 연동`.
- The source task is not automatically historical after linking. Selecting `S`
  continues `S`; selecting `L` continues `L`.
- The bridge uses one observer backbone and bounded per-thread command workers. The global live/poll sessions observe and read; they do not own linked-task writes.
- A worker may fork the first linked task or resume the canonical linked task for one Telegram turn. It releases after terminal persistence and delivery outbox enqueueing.
- `ready` means the bridge writer is released. It does not assert that Codex Desktop has closed the linked task.
- If Codex Desktop still owns the linked task, the Telegram command fails closed with close-and-retry guidance. The bridge does not create another fork, does not retain rejected plaintext prompt/transcript for a retry button, and leaves the link retryable.
- Historical linked task `L` remains preserved and usable when the operator
  explicitly selects `L` or replies to a message routed to `L`.
- Replies with explicit pre-link source turn routes continue through the
  canonical linked-task compatibility path.
- Closing or navigating away from the task may release the writer depending on the installed Codex build. If that build retains the writer, exiting Codex Desktop and retrying is the required operator action.
- Startup, recovery, and `/repair` perform no `thread/resume`, `thread/fork`, `turn/start`, or `thread/name/set` calls for source or linked tasks.
- Stale `telegram_running` after restart is conservatively recovered to `release_pending`; `release_pending` is not promoted to `ready` without exact current-daemon worker-close confirmation.
- `/repair` may clear ambiguous local creation state and expose pending naming retry to the next safe acquisition. It does not acquire, resume, rename, or start a linked writer itself.
- Telegram approval buttons are valid only for bridge-worker-owned turns with matching worker generation. A Desktop-owned approval may be notified in Telegram, but it must be answered in Codex Desktop.
- Voice execution is confirmation-gated. After the transcript is confirmed, routing follows the same canonical linked task and single-writer rules as text input.
- Linked lifecycle diagnostics may include event name, chat hash, non-sensitive project id, short source/link/turn ids, generation, state, duration, and RPC code. They must not include prompt text, titles, transcripts, full paths, full ids, tokens, or session identities.
...
## Windows minimal bridge acceptance boundary

| Contract | Deterministic coverage | Live qualification |
| --- | --- | --- |
| Project selection, text/reply FIFO, restart/retry, final reassembly | `tests/telegram_minimal_e2e_test.go` | Authorized private Telegram readback required |
| Approval | Fake App Server request/callback round trip | PC-origin approval is qualified only for a bridge-managed shared App Server session |
| Voice preview/cancel/execute | Fake Telegram file download and fake transcriber | ffmpeg/OpenAI/Telegram click-through required |
| Privacy | SQLite bytes and captured logs contain no unique transcript marker | No private ids, transcripts, tokens, or logs may be committed |
| Linked task handoff | `internal/daemon/service_test.go`, `internal/daemon/minimal_continuation_test.go`, `internal/daemon/minimal_observer_test.go`, and `internal/storage/store_minimal_test.go` linked ownership tests | Must verify writer release on the installed Codex Desktop build before claiming navigation/close is sufficient |

A stock native Codex Desktop task owns a separate App Server. Completion
reconciliation remains supported, but Telegram approval for that separate
Desktop App Server is not claimed.
...
- in legacy/non-minimal profiles, repair recreates app-server sessions and may resume tracked observer threads without manual route surgery; in minimal linked mode, startup/recovery/repair never call `thread/start`, `thread/resume`, `thread/fork`, `turn/start`, or `thread/name/set`, and the only linked-task ownership probe after repair is a later user Telegram command through a bounded worker
