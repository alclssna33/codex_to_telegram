...
import (
	...
	"github.com/alclssna33/codex_to_telegram/internal/version"
)

type Event = control.Event
type TurnStartOptions = control.TurnStartOptions
type SandboxPolicy = control.SandboxPolicy
...
func CommandExitConfirmed(err error) bool {
	if err == nil {
		return false
	}
	var exitErr *CommandExitError
	return errors.As(err, &exitErr) && exitErr.Confirmed
}
...
func (c *Client) Start(ctx context.Context) error {
	...
	if _, err := c.Request(ctx, "initialize", map[string]any{
		...
		_ = c.closeRunning(context.Background())
		return err
	}
...
func (c *Client) CloseContext(ctx context.Context) error {
	...
	c.startMu.Lock()
	defer c.startMu.Unlock()
	return c.closeRunning(ctx)
}

func (c *Client) closeRunning(ctx context.Context) error {
	c.mu.Lock()
	if !c.started {
		c.mu.Unlock()
		return nil
	...
	c.serverRequests = map[string]serverRequest{}
	c.started = false
	c.generation++
	c.cmd = nil
	c.stdin = nil
	...
	if cmd != nil && cmd.Process != nil {
		return waitForCommandExit(ctx, cmd)
	}
	return nil
}

func waitForCommandExit(ctx context.Context, cmd *exec.Cmd) error {
	if cmd == nil || cmd.Process == nil {
		return nil
	}
	...
	case err := <-waited:
		if err != nil {
			return &CommandExitError{Err: err, Confirmed: true}
		}
		return nil
	case <-ctx.Done():
		timeoutErr := ctx.Err()
		...
		case waitErr := <-waited:
			err := timeoutErr
			if killErr != nil {
				err = errors.Join(timeoutErr, killErr, waitErr)
			} else if waitErr != nil {
				err = errors.Join(timeoutErr, waitErr)
			}
			return &CommandExitError{Err: err, Confirmed: true, Forced: true}
		case <-time.After(5 * time.Second):
			return errors.Join(timeoutErr, killErr, errors.New("app-server process was not reaped after forced kill"))
		}
	}
}
...
func (c *Client) Request(ctx context.Context, method string, params map[string]any) (any, error) {
	...
	case <-time.After(timeout):
		...
		delete(c.pending, id)
		c.mu.Unlock()
		return nil, fmt.Errorf("request timeout for %s: %w", method, context.DeadlineExceeded)
	case <-ctx.Done():
		c.mu.Lock()
		delete(c.pending, id)
		c.mu.Unlock()
...
func (c *Client) RespondServerRequest(ctx context.Context, requestID string, result map[string]any) error {
	...
	request, ok := c.serverRequests[requestID]
	if !ok || request.generation != c.generation {
		c.mu.Unlock()
		return errors.New("app-server request is not pending in the current session")
	}
	stdin := c.stdin
	...
	_, writeErr := io.WriteString(stdin, string(payload)+"\n")
	if writeErr == nil {
		delete(c.serverRequests, requestID)
	}
	c.mu.Unlock()
	if writeErr != nil {
		c.broadcast(Event{Channel: "transport_error", Params: map[string]any{"stream": "stdin", "method": "serverRequest/respond", "error": writeErr.Error(), "stderr_tail": c.StderrTail()}})
		return writeErr
	}
	return nil
}

func (c *Client) RespondServerRequestExact(ctx context.Context, requestID, threadID, turnID, requestKind string, result map[string]any) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	c.mu.Lock()
	...
	if request.threadID != strings.TrimSpace(threadID) || request.turnID != strings.TrimSpace(turnID) || request.requestKind != strings.TrimSpace(requestKind) {
		c.mu.Unlock()
...
func (c *Client) ThreadList(ctx context.Context, limit int, cursor string) (map[string]any, error) {
	return c.ThreadListWithOptions(ctx, control.ThreadListOptions{
		Limit:   limit,
		Cursor:  cursor,
		SortKey: "updated_at",
	})
...
func (c *Client) ThreadListWithOptions(ctx context.Context, options control.ThreadListOptions) (map[string]any, error) {
	result, err := c.Request(ctx, "thread/list", threadListParams(options))
	if err != nil {
		return nil, err
	}
...
func threadListParams(options control.ThreadListOptions) map[string]any {
	limit := options.Limit
	if limit <= 0 {
		limit = 25
	}
	params := map[string]any{"limit": limit}
	if value := strings.TrimSpace(options.Cursor); value != "" {
		params["cursor"] = value
	}
	if value := strings.TrimSpace(options.SortKey); value != "" {
		params["sortKey"] = value
	}
	if value := strings.TrimSpace(options.SortDirection); value != "" {
...
func (c *Client) ThreadFork(ctx context.Context, threadID string, options control.ThreadForkOptions) (map[string]any, error) {
	result, err := c.Request(ctx, "thread/fork", threadForkParams(threadID, options))
	if err != nil {
		return nil, err
	}
	return asMap(result), nil
...
func threadForkParams(threadID string, options control.ThreadForkOptions) map[string]any {
	params := map[string]any{"threadId": strings.TrimSpace(threadID)}
	if cwd := strings.TrimSpace(options.CWD); cwd != "" {
		params["cwd"] = cwd
	}
	if turnID := strings.TrimSpace(options.LastTurnID); turnID != "" {
		params["lastTurnId"] = turnID
	}
	return params
}
...
func turnStartParams(threadID, message, cwd string, options TurnStartOptions) (map[string]any, error) {
	...
	if policy := strings.TrimSpace(options.ApprovalPolicy); policy != "" {
		params["approvalPolicy"] = policy
	}
	if policyType := strings.TrimSpace(options.SandboxPolicy.Type); policyType != "" {
		params["sandboxPolicy"] = map[string]any{
			"type":          policyType,
			"writableRoots": append([]string(nil), options.SandboxPolicy.WritableRoots...),
			"networkAccess": options.SandboxPolicy.NetworkAccess,
		}
	}
	...
	for {
		...
		if len(line) == 0 && readErr != nil {
			...
			if !errors.Is(readErr, io.EOF) {
				c.broadcast(Event{Channel: "transport_error", Params: map[string]any{"stream": "stdout", "generation": generation, "error": readErr.Error(), "stderr_tail": c.StderrTail()}})
				return
			}
...
func (c *Client) handlePayload(payload map[string]any, generation uint64) {
	if id, ok := payload["id"]; ok {
		if _, hasResult := payload["result"]; hasResult || payload["error"] != nil {
			...
			if reply != nil {
				if payload["error"] != nil {
					reply <- rpcResponse{Error: rpcErrorFromPayload(payload["error"])}
				} else {
		...
		if method, ok := payload["method"].(string); ok {
			...
			c.serverRequests[requestID] = serverRequest{
				rawID: id, generation: generation,
				threadID:    strings.TrimSpace(stringValue(params["threadId"], "")),
				turnID:      strings.TrimSpace(stringValue(params["turnId"], "")),
				requestKind: strings.TrimSpace(method),
			}
			c.mu.Unlock()
			// Preserve the wire ID unchanged: approval routing must answer the
			// originating App Server request rather than a synthesized identifier.
...
func rpcErrorFromPayload(value any) error {
	payload := asMap(value)
	return &RPCError{
		Code:    int(int64Value(payload["code"])),
		Message: strings.TrimSpace(stringValue(payload["message"], "app-server request failed")),
	}
}
