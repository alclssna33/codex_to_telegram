...
type FilteredThreads interface {
	ThreadListWithOptions(ctx context.Context, options ThreadListOptions) (map[string]any, error)
}

type ThreadAdmin interface {
	ThreadFork(ctx context.Context, threadID string, options ThreadForkOptions) (map[string]any, error)
