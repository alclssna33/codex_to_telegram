...
func ScopedRequestID(sessionIdentity, wireRequestID string) string {
	sessionIdentity = strings.TrimSpace(sessionIdentity)
	wireRequestID = strings.TrimSpace(wireRequestID)
	if sessionIdentity == "" || wireRequestID == "" {
		return wireRequestID
	}
	sum := sha256.Sum256([]byte(sessionIdentity + "\x00" + wireRequestID))
	return fmt.Sprintf("scoped-request:%x", sum[:])
}

func NormalizeRequestIdentity(sessionIdentity, requestID, wireRequestID string) (string, string) {
	sessionIdentity = strings.TrimSpace(sessionIdentity)
	requestID = strings.TrimSpace(requestID)
	wireRequestID = strings.TrimSpace(wireRequestID)
	hasExplicitWireRequestID := wireRequestID != ""
	if wireRequestID == "" {
	...
	if sessionIdentity == "" {
		...
		return requestID, wireRequestID
	}
	if !hasExplicitWireRequestID {
		return requestID, wireRequestID
	}
	if wireRequestID == "" {
		return "", ""
	}
	return ScopedRequestID(sessionIdentity, wireRequestID), wireRequestID
}
