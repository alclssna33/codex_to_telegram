...
// NewBotWithClient uses the same Bot runtime with an explicitly supplied API
// client. Production callers should normally use NewBot; the explicit client
// keeps local HTTP integration tests off the real Telegram API.
func NewBotWithClient(cfg config.Config, service *daemon.Service, logger *log.Logger, client *Client) (*Bot, error) {
	if client == nil {
		return nil, errors.New("telegram client is required")
	}
	bot, err := NewBot(cfg, service, logger)
	if err != nil {
		return nil, err
	}
	bot.client = client
...
func (b *Bot) Start(ctx context.Context) error {
	...
	b.me = me
	if err := b.client.SetMyCommands(startCtx, commandsForProfile(b.cfg.Profile)); err != nil {
...
// HandleUpdate processes one already-received Telegram update. Run remains
// responsible for long polling in production.
func (b *Bot) HandleUpdate(ctx context.Context, update Update) error {
...
func (b *Bot) handleMessage(ctx context.Context, message Message) error {
	...
	if isOwnerOnlyProfile(b.cfg.Profile) && !b.service.IsAllowed(message.From.ID, message.Chat.ID) {
		return nil
	}
	if isMinimalProfile(b.cfg.Profile) && message.Voice != nil {
	...
	response, err := b.service.HandleInboundText(ctx, model.InboundText{
...
func (b *Bot) handleVoiceMessage(ctx context.Context, message Message) error {
	replyTo := int64(0)
	if message.ReplyToMessage != nil {
		replyTo = message.ReplyToMessage.MessageID
	}
	voice := message.Voice
	response, err := b.service.HandleVoice(ctx, daemon.VoiceInput{
		ChatID:           message.Chat.ID,
		TopicID:          message.MessageThreadID,
		UserID:           message.From.ID,
		MessageID:        message.MessageID,
		...
		Open: func(openCtx context.Context) (io.ReadCloser, error) {
			file, err := b.client.GetFile(openCtx, voice.FileID)
			if err != nil {
				return nil, errors.New("telegram voice download failed")
			}
			body, err := b.client.DownloadFile(openCtx, file.FilePath)
			if err != nil {
				return nil, errors.New("telegram voice download failed")
			}
			return body, nil
...
func (b *Bot) handleCallback(ctx context.Context, callback CallbackQuery) error {
	...
	if isOwnerOnlyProfile(b.cfg.Profile) && !b.service.IsAllowed(callback.From.ID, chatID) {
		return nil
	}
	messageID := int64(0)
	if callback.Message != nil {
...
func (b *Bot) deliverDirectResponse(ctx context.Context, chatID, topicID int64, response *daemon.DirectResponse) error {
	...
	if err != nil {
		_ = b.service.AbandonVoiceConfirmation(ctx, response.VoiceConfirmationID)
		return err
	}
...
func commandsForProfile(profile string) []BotCommand {
	if isMinimalProfile(profile) {
		return []BotCommand{{Command: "start", Description: "작업폴더 선택"}}
	}
	if isNotifierProfile(profile) {
		return []BotCommand{
...
func isMinimalProfile(profile string) bool {
	return strings.EqualFold(strings.TrimSpace(profile), "minimal")
}
...
func isOwnerOnlyProfile(profile string) bool {
	return isMinimalProfile(profile) || isNotifierProfile(profile)
}
