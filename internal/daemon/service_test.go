...
type failingDeliverySender struct {
	*recordingSender
	err error
}

func (s *failingDeliverySender) SendMessage(context.Context, int64, int64, string, [][]model.ButtonSpec, model.SendOptions) (int64, error) {
	return 0, s.err
}

func TestProcessDeliveryBatchPersistsOnlySafeErrorMetadata(t *testing.T) {
	service := newTestService(t)
	service.cfg.DeliveryMaxAttempts = 3
	service.cfg.DeliveryRetryBase = time.Second
	ctx := context.Background()
	privateMarker := "private response marker"
	fakeTokenURL := "https://api.telegram.org/bot123456789:AAEfakeTokenValue1234567890/sendMessage"
	transportError := "POST " + fakeTokenURL + " returned 401 body=" + privateMarker
	service.SetSender(&failingDeliverySender{
	...
	db, err := sql.Open("sqlite", service.store.Path())
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	var attemptError, queueError string
	if err := db.QueryRowContext(ctx, `SELECT error_text FROM delivery_attempts ORDER BY id DESC LIMIT 1`).Scan(&attemptError); err != nil {
		t.Fatal(err)
	}
	if err := db.QueryRowContext(ctx, `SELECT last_error FROM delivery_queue WHERE event_id = 'delivery-error-e1'`).Scan(&queueError); err != nil {
		t.Fatal(err)
	}
	stateError, err := service.store.GetState(ctx, "daemon.last_error")
	if err != nil {
		t.Fatal(err)
	}
	expected := "class=telegram_delivery status=send_error length=124 sha256=06970fb9aec0"
	for name, value := range map[string]string{
		"delivery attempt": attemptError,
		"delivery queue":   queueError,
		"daemon state":     stateError,
	} {
		if value != expected {
			t.Fatalf("%s error metadata = %q, want %q", name, value, expected)
		}
	}
	for _, path := range []string{service.store.Path(), service.store.Path() + "-wal", service.store.Path() + "-shm"} {
		...
		if bytes.Contains(raw, []byte(fakeTokenURL)) || bytes.Contains(raw, []byte(privateMarker)) {
			t.Fatalf("delivery error content leaked to %s", filepath.Base(path))
		}
	}
}

func TestNewMinimalProfileProtectsDeliveryPayloadAtRest(t *testing.T) {
	...
	secret := "daemon minimal delivery marker"
	if err := service.store.EnqueueDelivery(context.Background(), model.DeliveryQueueItem{
		EventID:     "e1",
		ChatKey:     "1:0",
		Status:      model.DeliveryStatusPending,
		PayloadJSON: secret,
	...
	for _, path := range []string{cfg.Paths.DBPath, cfg.Paths.DBPath + "-wal", cfg.Paths.DBPath + "-shm"} {
		...
		if bytes.Contains(raw, []byte(secret)) {
			t.Fatalf("minimal profile stored plaintext delivery payload in %s", filepath.Base(path))
		}
	}
}

func TestOpenStoreForMinimalProfileRejectsNonWindows(t *testing.T) {
	root := t.TempDir()
	cfg := config.Config{
		Profile: "minimal",
		Paths: config.Paths{
	...
	store, err := openStoreForProfile(cfg, "linux")
	if store != nil {
		_ = store.Close()
		t.Fatal("minimal non-Windows storage unexpectedly opened")
	}
	if err == nil || !strings.Contains(err.Error(), "minimal profile requires Windows DPAPI") {
		t.Fatalf("error = %v, want clear unsupported-platform error", err)
	}
}

func TestNotifierServiceOpenStoreRejectsNonWindowsProtectedStore(t *testing.T) {
	root := t.TempDir()
	cfg := config.Config{
		Profile: "notifier",
		Paths: config.Paths{
	...
	if store != nil {
		_ = store.Close()
		t.Fatal("notifier non-Windows storage unexpectedly opened")
	}
	if err == nil || !strings.Contains(err.Error(), "notifier profile requires Windows DPAPI") {
		t.Fatalf("error = %v, want clear notifier DPAPI error", err)
	}
}

func TestNotifierServiceOmitsInteractiveComponents(t *testing.T) {
	cfg := minimalStartupTestConfig(t)
	cfg.Profile = "notifier"
	cfg.Projects = nil
	if err := cfg.Paths.Ensure(); err != nil {
		t.Fatal(err)
	}
	store, err := storage.OpenWithProtector(cfg.Paths.DBPath, securestore.NewDeterministicTestProtector())
	if err != nil {
		t.Fatal(err)
	}
	service, err := newWithStore(cfg, store)
	if err != nil {
		_ = store.Close()
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = service.Close() })
	if service.projectRegistry != nil || service.minimalRouter != nil || service.minimalWorkers != nil || service.voiceTranscriber != nil || service.live != nil {
		t.Fatalf("interactive components were constructed: %#v", service)
	}
	if service.poll == nil {
		t.Fatal("notifier poll session is nil")
	}
	if service.liveFactory != nil || service.minimalWorkerFactory != nil {
		t.Fatalf("interactive factories were constructed: live=%v worker=%v", service.liveFactory != nil, service.minimalWorkerFactory != nil)
	}
	if service.pollFactory == nil {
		t.Fatal("notifier poll factory is nil")
	}
	if service.minimalApprovalSessionID != "" || service.voiceSessionIdentity != "" {
		t.Fatalf("minimal session identities were created: approval=%q voice=%q", service.minimalApprovalSessionID, service.voiceSessionIdentity)
	}
}

func TestNotifierServiceEnsureSessionLifecycleStartsPollOnly(t *testing.T) {
	service := newNotifierServiceFromConstructor(t)
	ctx := context.Background()
	live := &lifecycleCountingSession{}
	poll := &lifecycleCountingSession{}
	service.live = live
	service.poll = poll

	service.ensureSessionLifecycle(ctx)

	if live.Starts() != 0 {
		t.Fatalf("live starts = %d, want 0", live.Starts())
	}
	if poll.Starts() != 1 {
		t.Fatalf("poll starts = %d, want 1", poll.Starts())
	}
	service.mu.RLock()
	liveConnected := service.liveConnected
	pollConnected := service.pollConnected
	service.mu.RUnlock()
	if liveConnected || !pollConnected {
		t.Fatalf("connected live=%t poll=%t, want live false poll true", liveConnected, pollConnected)
	}
}

func TestNotifierServiceRepairRecreatesPollOnlyAndLeavesApprovalsPending(t *testing.T) {
	service := newNotifierServiceFromConstructor(t)
	ctx := context.Background()
	if err := service.store.SavePendingApproval(ctx, model.PendingApproval{
		RequestID:   "notifier-approval",
	...
	oldLive := &lifecycleCountingSession{}
	oldPoll := &lifecycleCountingSession{}
	newLive := &lifecycleCountingSession{}
	newPoll := &lifecycleCountingSession{}
	liveFactoryCalls := 0
	pollFactoryCalls := 0
	...
	service.poll = oldPoll
	service.liveConnected = true
	service.pollConnected = true

	service.repairSessions(ctx, "notifier-test")

	if liveFactoryCalls != 0 || newLive.Starts() != 0 || oldLive.Closes() != 0 {
		t.Fatalf("live repair activity factory=%d new_starts=%d old_closes=%d, want all zero", liveFactoryCalls, newLive.Starts(), oldLive.Closes())
	}
	if pollFactoryCalls != 1 || newPoll.Starts() != 1 || oldPoll.Closes() != 1 {
		t.Fatalf("poll repair activity factory=%d new_starts=%d old_closes=%d, want 1 each", pollFactoryCalls, newPoll.Starts(), oldPoll.Closes())
	}
	approval, err := service.store.GetPendingApproval(ctx, "notifier-approval")
	if err != nil {
		t.Fatal(err)
	}
	if approval == nil || approval.Status != "pending" {
		t.Fatalf("approval = %#v, want still pending", approval)
	}
}

func TestNotifierControlLoopRepairsPollWithBoundedContextClose(t *testing.T) {
	service := newNotifierServiceFromConstructor(t)
	service.cfg.RequestTimeout = 25 * time.Millisecond
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	oldPoll := newBlockingCloseContextSession()
	defer oldPoll.release()
	newPoll := newStartSignalingSession()
	service.pollFactory = func() Session { return newPoll }
	service.mu.Lock()
	service.poll = oldPoll
	service.pollConnected = true
	service.mu.Unlock()
	if err := service.store.SetState(ctx, "control.repair_request", "2026-08-25T00:00:00Z|notifier_thread_list"); err != nil {
		t.Fatal(err)
	}

	controlDone := make(chan struct{})
	go func() {
		service.controlLoop(ctx)
		close(controlDone)
	}()

	select {
	case <-newPoll.started:
	case <-time.After(250 * time.Millisecond):
		t.Fatal("notifier repair did not replace the blocking poll session before the test deadline")
	}
	if !oldPoll.closeContextCalled() {
		t.Fatal("notifier repair did not use the old poll session's bounded CloseContext")
	}
	...
	for {
		...
		select {
		case <-deadline.C:
			t.Fatalf("control.repair_request = %q, want cleared after repair", repair)
	...
	case <-time.After(time.Second):
		t.Fatal("control loop did not stop after cancellation")
	}
}

func TestNotifierRepairStartsReplacementBeforeSlowOldCloseFinishes(t *testing.T) {
	service := newNotifierServiceFromConstructor(t)
	service.cfg.RequestTimeout = time.Second
	ctx := context.Background()
	oldPoll := newBlockingCloseContextSession()
	defer oldPoll.release()
	newPoll := newStartSignalingSession()
	...
	go func() {
		service.repairNotifierPollSession(ctx, "notifier-test")
		close(repairDone)
	}()
	select {
	case <-newPoll.started:
	case <-time.After(100 * time.Millisecond):
		t.Fatal("notifier replacement waited for a slow old poll close")
	}
	if !oldPoll.closeContextCalled() {
		t.Fatal("notifier repair did not begin bounded close for old poll")
	}
	oldPoll.release()
	select {
	case <-repairDone:
	case <-time.After(time.Second):
		t.Fatal("notifier repair did not finish after old poll release")
	}
}

func TestNotifierRepairWaitsForInFlightObservationBeforeClosingPoll(t *testing.T) {
	service := newNotifierServiceFromConstructor(t)
	ctx := context.Background()
	oldPoll := newBlockingNotifierListSession()
	newPoll := &lifecycleCountingSession{}
	service.pollFactory = func() Session { return newPoll }
	service.mu.Lock()
	service.poll = oldPoll
	...
	go func() {
		service.runNotifierObservationCycle(ctx)
		close(observationDone)
	}()
	select {
	case <-oldPoll.listStarted:
	case <-time.After(time.Second):
		t.Fatal("notifier observation did not begin its list request")
	}
	...
	select {
	case <-oldPoll.closeCalled:
		t.Fatal("notifier repair closed the poll session while its observation was still reading")
	...
	case <-time.After(time.Second):
		t.Fatal("notifier observation did not finish after its list response was released")
	}
	select {
	case <-repairDone:
	case <-time.After(time.Second):
		t.Fatal("notifier repair did not complete after the observation finished")
	}
	if oldPoll.Closes() != 1 {
		t.Fatalf("old poll close count = %d, want 1 after the observation completed", oldPoll.Closes())
	}
}

func TestNotifierListReadHonorsConfiguredRequestTimeout(t *testing.T) {
	service := newNotifierServiceFromConstructor(t)
	service.cfg.RequestTimeout = 30 * time.Second
	session := &deadlineCapturingNotifierListSession{}
	before := time.Now()
	if _, err := service.readNotifierListPage(context.Background(), session, ""); err != nil {
		t.Fatalf("readNotifierListPage() error = %v", err)
	}
	if session.deadline.Before(before.Add(29 * time.Second)) {
		t.Fatalf("notifier list deadline = %s, want it to honor the 30s configured request timeout", session.deadline)
	}
}

func TestCloseWaitsForKickBootstrapWork(t *testing.T) {
	service := newTestService(t)
	session := newBlockingBootstrapSession()
	...
	service.kickBootstrap()
	session.waitThreadListStarted(t)
	...
	case err := <-closeDone:
		...
		t.Fatal("Close returned while kickBootstrap work was still running")
	...
	case err := <-closeDone:
		if err != nil {
			t.Fatalf("Close after bootstrap release returned error: %v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("Close did not finish after kickBootstrap work was released")
	}
}

func TestCloseCancelsKickBootstrapWorkWithServiceContext(t *testing.T) {
	service := newTestService(t)
	session := newContextBlockingBootstrapSession()
	runCtx, cancel := context.WithCancel(context.Background())
	service.mu.Lock()
	service.started = true
	service.runCtx = runCtx
	...
	case <-time.After(time.Second):
		t.Fatal("Close did not cancel kickBootstrap work with the service context")
	}
	session.waitCanceled(t)
}

func TestCloseCancelsTelegramOriginHotPollWithServiceContext(t *testing.T) {
	service := newTestService(t)
	runCtx, cancel := context.WithCancel(context.Background())
	service.mu.Lock()
	service.started = true
	...
	defer callerCancel()
	service.startTelegramOriginHotPoll(callerCtx, "thread-hot", "turn-hot")
	...
	case <-time.After(250 * time.Millisecond):
		...
		case <-closeDone:
		case <-time.After(time.Second):
			t.Fatal("Close remained blocked after caller context cancellation")
		}
		t.Fatal("Close did not cancel telegram-origin hot poll with the service context")
	}
}

func TestStatusShowsDesktopContinuationParentAndChild(t *testing.T) {
	svc, _ := newMinimalService(t)
	ctx := context.Background()
	parentID, childID := "parent-12345678", "child-87654321"
	claim, created, err := svc.store.ClaimMinimalContinuation(ctx, model.MinimalContinuation{
		Key:       model.MinimalContinuationKey{ChatID: 7, SourceThreadID: parentID, SourceTurnID: "turn-1"},
		ProjectID: svc.cfg.Projects[0].ID,
	})
	if err != nil || !created {
		t.Fatalf("claim=%#v created=%t err=%v", claim, created, err)
	}
	child := model.Thread{ID: childID, CWD: svc.cfg.Projects[0].CanonicalPath, Status: "completed"}
	if err := svc.store.ActivateMinimalContinuation(ctx, *claim, child); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(status, "Mode: Telegram continuation") ||
		!strings.Contains(status, "Parent: parent-1") ||
		!strings.Contains(status, "Fork: child-87") ||
		!strings.Contains(status, "Anchor turn: turn-1") {
		t.Fatalf("status=%q", status)
	}
	if strings.Contains(status, child.CWD) || strings.Contains(status, parentID) || strings.Contains(status, childID) {
		t.Fatalf("status leaked private/full identity: %q", status)
	}
}

func TestStatusShowsNormalBindingAfterLeavingContinuationFork(t *testing.T) {
	svc, _ := newMinimalService(t)
	ctx := context.Background()
	claim, created, err := svc.store.ClaimMinimalContinuation(ctx, model.MinimalContinuation{
		Key:       model.MinimalContinuationKey{ChatID: 7, SourceThreadID: "parent-12345678", SourceTurnID: "turn-1"},
		ProjectID: svc.cfg.Projects[0].ID,
	})
	if err != nil || !created {
		t.Fatalf("claim=%#v created=%t err=%v", claim, created, err)
	}
	if err := svc.store.ActivateMinimalContinuation(ctx, *claim, model.Thread{ID: "child-87654321", CWD: svc.cfg.Projects[0].CanonicalPath, Status: "completed"}); err != nil {
		t.Fatal(err)
	}
	normal := model.Thread{ID: "normal-thread-9999", Title: "Normal", CWD: svc.cfg.Projects[0].CanonicalPath, Status: "completed"}
	if err := svc.store.UpsertThread(ctx, normal); err != nil {
		t.Fatal(err)
	}
	if err := svc.store.SetBinding(ctx, 7, 0, normal.ID, model.BindingModeBound); err != nil {
		t.Fatal(err)
	}
	...
	if strings.Contains(status, "Mode: Telegram continuation") || strings.Contains(status, "Fork: child-87") {
		t.Fatalf("status used stale continuation after normal binding: %q", status)
	}
	if !strings.Contains(status, "Mode: Bound thread") || !strings.Contains(status, normal.Label()) {
		t.Fatalf("status=%q, want normal binding", status)
	}
}

func TestStatusUsesBoundContinuationWhenMultipleAreActive(t *testing.T) {
	svc, _ := newMinimalService(t)
	ctx := context.Background()
	first, created, err := svc.store.ClaimMinimalContinuation(ctx, model.MinimalContinuation{
		Key:       model.MinimalContinuationKey{ChatID: 7, SourceThreadID: "parent-old-1234", SourceTurnID: "turn-old"},
		ProjectID: svc.cfg.Projects[0].ID,
	})
	if err != nil || !created {
		t.Fatalf("first=%#v created=%t err=%v", first, created, err)
	}
	if err := svc.store.ActivateMinimalContinuation(ctx, *first, model.Thread{ID: "child-old-1234", CWD: svc.cfg.Projects[0].CanonicalPath, Status: "completed"}); err != nil {
		t.Fatal(err)
	}
	second, created, err := svc.store.ClaimMinimalContinuation(ctx, model.MinimalContinuation{
		Key:       model.MinimalContinuationKey{ChatID: 7, SourceThreadID: "parent-new-5678", SourceTurnID: "turn-new"},
		ProjectID: svc.cfg.Projects[0].ID,
	})
	if err != nil || !created {
		t.Fatalf("second=%#v created=%t err=%v", second, created, err)
	}
	if err := svc.store.ActivateMinimalContinuation(ctx, *second, model.Thread{ID: "child-new-5678", CWD: svc.cfg.Projects[0].CanonicalPath, Status: "completed"}); err != nil {
		t.Fatal(err)
	}
	if err := svc.store.SetBinding(ctx, 7, 0, "child-old-1234", model.BindingModeBound); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(status, "Mode: Telegram continuation") ||
		!strings.Contains(status, "Parent: parent-o") ||
		!strings.Contains(status, "Fork: child-ol") ||
		!strings.Contains(status, "Anchor turn: turn-old") {
		t.Fatalf("status=%q, want bound old continuation", status)
	}
	if strings.Contains(status, "parent-n") || strings.Contains(status, "child-ne") || strings.Contains(status, "turn-new") {
		t.Fatalf("status selected latest unrelated continuation: %q", status)
	}
}

func TestStatusShowsLinkedTaskNameShortIDsAndOwnershipState(t *testing.T) {
	svc, _ := newMinimalService(t)
	ctx := context.Background()
	sourceID := "source-status-123456"
	linkedID := "linked-status-abcdef"
	seedReadyLinkedThread(t, svc, 7, 0, sourceID, "source-turn-123456", linkedID)
	if err := svc.store.RecordMinimalLinkedBlocked(ctx, linkedID, "active_writer_with_extra_private_detail_that_must_be_bounded", svc.now()); err != nil {
		t.Fatal(err)
	}
	...
	for _, want := range []string{
		"Mode: Telegram linked task",
		"Task: 안클코 · 텔레그램 연동",
		"Source: source-s",
		"Linked: linked-s",
		"State: bridge released",
		"Last acquire: blocked by another writer",
	} {
		if !strings.Contains(status, want) {
			t.Fatalf("status missing %q in:\n%s", want, status)
		}
	}
	for _, forbidden := range []string{sourceID, linkedID, svc.cfg.Projects[0].CanonicalPath, "active_writer_with_extra_private_detail", "Codex is closed"} {
		if strings.Contains(status, forbidden) {
			t.Fatalf("status leaked forbidden value %q in:\n%s", forbidden, status)
		}
	}
}

func TestLinkedStatusOmitsBlockedNoticeWhenBlockedAtMalformed(t *testing.T) {
	link := model.MinimalLinkedThread{
		SourceThreadID:  "source-status-123456",
		LinkedThreadID:  "linked-status-abcdef",
		DesiredTitle:    "안클코 · 텔레그램 연동",
		State:           model.MinimalLinkedReady,
		LastBlockedAt:   model.TimeString("not-a-time"),
		LastBlockedCode: "active_writer",
	}

	lines := minimalLinkedStatusLines(link, time.Date(2026, 8, 23, 5, 0, 0, 0, time.UTC))

	for _, line := range lines {
		if strings.Contains(line, "Last acquire") {
			t.Fatalf("status lines=%#v, want malformed blocked timestamp omitted", lines)
		}
	}
}

func TestMinimalServiceStartupAdoptsLinkedBeforeClaimedCommandRecovery(t *testing.T) {
	cfg := minimalStartupTestConfig(t)
	ctx := context.Background()
	service, err := New(cfg)
	if err != nil {
	...
	seedLegacyMinimalContinuation(t, service, 7, 0, project.ID, project.CanonicalPath, "source-startup", "turn-startup", "linked-startup")
	if err := service.store.EnqueuePendingCommand(ctx, model.PendingCommand{
		ThreadID: "linked-startup", SourceThreadID: "source-startup", SourceTurnID: "turn-startup",
		ProjectID: project.ID, ChatID: 7, Prompt: "restart claimed prompt",
	}); err != nil {
		t.Fatal(err)
	}
	if claimed, err := service.store.ClaimPendingCommand(ctx, "linked-startup"); err != nil || claimed == nil {
		t.Fatalf("ClaimPendingCommand = %#v, %v", claimed, err)
	}
	seedRunningMinimalLink(t, service, 8, 0, project.ID, project.CanonicalPath, "source-running", "turn-running", "linked-running", "linked-turn-running", 11)
	if err := service.Close(); err != nil {
		t.Fatal(err)
	}
...
CREATE TRIGGER claimed_recovery_requires_startup_adoption
BEFORE UPDATE OF status ON pending_commands
WHEN old.status = 'claimed' AND new.status = 'failed'
BEGIN
	SELECT CASE WHEN NOT EXISTS (
		SELECT 1 FROM minimal_linked_threads
		WHERE chat_key = '7:0' AND source_thread_id = 'source-startup'
	) THEN RAISE(ABORT, 'linked adoption must precede claimed recovery') END;
END;`); err != nil {
		_ = db.Close()
		t.Fatal(err)
	}
	...
	adopted, err := restarted.store.GetMinimalLinkedThread(ctx, 7, 0, "source-startup")
	if err != nil {
		t.Fatal(err)
	}
	if adopted == nil || adopted.LinkedThreadID != "linked-startup" || adopted.State != model.MinimalLinkedReady {
		t.Fatalf("adopted link=%#v, want ready linked-startup", adopted)
	}
	recovered, err := restarted.store.GetMinimalLinkedThreadByLinkedID(ctx, "linked-running")
	if err != nil {
		t.Fatal(err)
	}
	if recovered == nil || recovered.State != model.MinimalLinkedReleasePending || recovered.ActiveTurnID != "linked-turn-running" || recovered.WorkerGeneration != 11 {
		t.Fatalf("recovered running link=%#v, want conservative release_pending", recovered)
	}
	if recovered.State == model.MinimalLinkedTelegramRunning {
		t.Fatalf("startup kept linked thread running: %#v", recovered)
	}
}

func TestMinimalServiceStartupDoesNotResumeTrackedContinuations(t *testing.T) {
	service, _ := newMinimalService(t)
	ctx := context.Background()
	project := service.cfg.Projects[0]
	seedLegacyMinimalContinuation(t, service, 7, 0, project.ID, project.CanonicalPath, "source-start", "turn-start", "linked-start")
	live := &stubSession{}
	poll := &stubSession{threadReadErr: errors.New("thread not loaded")}
	service.live = live
	service.poll = poll
	service.liveConnected = false
	service.pollConnected = false

	service.ensureSessions(ctx)

	if len(live.threadResumeCalls) != 0 || len(poll.threadResumeCalls) != 0 {
		t.Fatalf("startup resume calls: live=%#v poll=%#v, want none", live.threadResumeCalls, poll.threadResumeCalls)
	}
	if len(live.threadStartCalls) != 0 || len(poll.threadStartCalls) != 0 {
		t.Fatalf("startup thread starts: live=%#v poll=%#v, want none", live.threadStartCalls, poll.threadStartCalls)
	}
	if len(live.turnStartCalls) != 0 || len(poll.turnStartCalls) != 0 {
		t.Fatalf("startup turn starts: live=%#v poll=%#v, want none", live.turnStartCalls, poll.turnStartCalls)
	}
}

func TestMinimalRecoveryDoesNotResume(t *testing.T) {
	service, _ := newMinimalService(t)
	ctx := context.Background()
	project := service.cfg.Projects[0]
	seedLegacyMinimalContinuation(t, service, 7, 0, project.ID, project.CanonicalPath, "source-repair", "turn-repair", "linked-repair")
	live := &stubSession{}
	poll := &stubSession{threadReadErr: errors.New("thread not loaded")}
	service.liveFactory = func() Session { return live }
	service.pollFactory = func() Session { return poll }

	service.repairSessions(ctx, "telegram")

	if len(live.threadResumeCalls) != 0 || len(poll.threadResumeCalls) != 0 {
		t.Fatalf("repair resume calls: live=%#v poll=%#v, want none", live.threadResumeCalls, poll.threadResumeCalls)
	}
	if len(live.threadStartCalls) != 0 || len(poll.threadStartCalls) != 0 {
		t.Fatalf("repair thread starts: live=%#v poll=%#v, want none", live.threadStartCalls, poll.threadStartCalls)
	}
	if len(live.turnStartCalls) != 0 || len(poll.turnStartCalls) != 0 {
		t.Fatalf("repair turn starts: live=%#v poll=%#v, want none", live.turnStartCalls, poll.turnStartCalls)
	}
}

func TestRepairDoesNotAcquireLinked(t *testing.T) {
	service, _ := newMinimalService(t)
	ctx := context.Background()
	workers := installWorkerFactory(service)
	worker, err := service.minimalWorkers.Acquire(ctx, "link:linked-repair-title", "linked-repair-title")
	if err != nil {
		t.Fatal(err)
	}
	project := service.cfg.Projects[0]
	seedRunningMinimalLink(t, service, 7, 0, project.ID, project.CanonicalPath, "source-repair-title", "turn-repair-title", "linked-repair-title", "linked-turn-title", worker.Generation)

	response, err := service.handleCommand(ctx, 7, 0, "/repair", 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	session := workers.Single(t)
	if got := session.callSequence(); len(got) != 0 {
		t.Fatalf("/repair worker RPC sequence=%#v, want none", got)
	}
	if got := session.ThreadStartCalls(); got != 0 {
		t.Fatalf("/repair thread starts=%d, want none", got)
	}
	if got := session.ThreadResumeCalls(); got != 0 {
		t.Fatalf("/repair thread resumes=%d, want none", got)
	}
	if got := session.forkCallCount(); got != 0 {
		t.Fatalf("/repair thread forks=%d, want none", got)
	}
	if got := len(session.turnCalls()); got != 0 {
		t.Fatalf("/repair turn starts=%d, want none", got)
	}
	if got := session.ThreadSetNameCalls(); got != 0 {
		t.Fatalf("/repair thread name sets=%d, want none", got)
	}
	link, err := service.store.GetMinimalLinkedThreadByLinkedID(ctx, "linked-repair-title")
	if err != nil {
		t.Fatal(err)
	}
	if link == nil || link.TitleState != model.MinimalLinkedTitlePending || link.State != model.MinimalLinkedTelegramRunning {
		t.Fatalf("repair link=%#v, want pending title left for next safe acquisition", link)
	}
}

func TestRepairLinkedReleasePendingWithConfirmedReceiptFinalizesExactGeneration(t *testing.T) {
	service, app := newMinimalService(t)
	ctx := context.Background()
	workers := installWorkerFactory(service)
	project := service.cfg.Projects[0]
	const generation uint64 = 9
	seedRunningMinimalLink(t, service, 7, 0, project.ID, project.CanonicalPath, "source-repair-release", "turn-source", "linked-repair-release", "turn-linked", generation)
	if changed, err := service.store.BeginMinimalLinkedRelease(ctx, model.MinimalLinkedRelease{LinkedThreadID: "linked-repair-release", TurnID: "turn-linked", WorkerGeneration: generation}); err != nil || !changed {
		t.Fatalf("begin release changed=%t err=%v, want true nil", changed, err)
	}
	service.minimalWorkers.recordConfirmedRelease("linked-repair-release", generation, "minimal-link-worker:test:9")
	...
	link, err := service.store.GetMinimalLinkedThreadByLinkedID(ctx, "linked-repair-release")
	if err != nil {
		t.Fatal(err)
	}
	if link == nil || link.State != model.MinimalLinkedReady || link.ActiveTurnID != "" || link.WorkerGeneration != 0 {
		t.Fatalf("repair link=%#v, want ready with cleared turn/generation", link)
	}
	if count := countDeliveryKindStatus(t, service, "handoff_ready", model.DeliveryStatusPending); count != 1 {
		t.Fatalf("handoff_ready pending deliveries=%d, want 1", count)
	}
	if service.minimalWorkers.ConfirmedRelease("linked-repair-release", generation) {
		t.Fatal("repair left consumed confirmed-release receipt")
	}
	if got := len(workers.Sessions()); got != 0 {
		t.Fatalf("/repair acquired worker sessions=%d, want none", got)
	}
	if len(app.threadResumeCalls) != 0 || len(app.threadStartCalls) != 0 || len(app.turnStartCalls) != 0 {
		t.Fatalf("/repair writer RPCs resume=%#v threadStart=%#v turnStart=%#v, want none", app.threadResumeCalls, app.threadStartCalls, app.turnStartCalls)
	}
}

func TestRepairLinkedReleasePendingWithoutExactReceiptLeavesRowUnchanged(t *testing.T) {
	service, app := newMinimalService(t)
	ctx := context.Background()
	workers := installWorkerFactory(service)
	project := service.cfg.Projects[0]
	const generation uint64 = 11
	seedRunningMinimalLink(t, service, 7, 0, project.ID, project.CanonicalPath, "source-repair-no-receipt", "turn-source", "linked-repair-no-receipt", "turn-linked", generation)
	if changed, err := service.store.BeginMinimalLinkedRelease(ctx, model.MinimalLinkedRelease{LinkedThreadID: "linked-repair-no-receipt", TurnID: "turn-linked", WorkerGeneration: generation}); err != nil || !changed {
		t.Fatalf("begin release changed=%t err=%v, want true nil", changed, err)
	}
	service.minimalWorkers.recordConfirmedRelease("linked-repair-no-receipt", generation+1, "minimal-link-worker:test:12")

	if _, err := service.handleCommand(ctx, 7, 0, "/repair", 0); err != nil {
		t.Fatal(err)
	}

	link, err := service.store.GetMinimalLinkedThreadByLinkedID(ctx, "linked-repair-no-receipt")
	if err != nil {
		t.Fatal(err)
	}
	if link == nil || link.State != model.MinimalLinkedReleasePending || link.ActiveTurnID != "turn-linked" || link.WorkerGeneration != generation {
		t.Fatalf("repair link=%#v, want release_pending generation %d unchanged", link, generation)
	}
	if count := countDeliveryKind(t, service, "handoff_ready"); count != 0 {
		t.Fatalf("handoff_ready deliveries=%d, want none without exact receipt", count)
	}
	if got := len(workers.Sessions()); got != 0 {
		t.Fatalf("/repair acquired worker sessions=%d, want none", got)
	}
...
func TestServiceCloseClosesWorkersThenLivePoll(t *testing.T) {
	service, _ := newMinimalService(t)
	recorder := &closeOrderRecorder{}
	factory := newWorkerSessionFactory()
	factory.SetCloseHook(func(id string) { recorder.Add("worker:" + id) })
	service.minimalWorkers = newMinimalLinkWorkerManager(factory.New, time.Second, service.logLifecycle)
	if _, err := service.minimalWorkers.Acquire(context.Background(), "link:close-a", "linked-close-a"); err != nil {
		t.Fatal(err)
	}
	if _, err := service.minimalWorkers.Acquire(context.Background(), "link:close-b", "linked-close-b"); err != nil {
		t.Fatal(err)
	}
	service.live = &orderedCloseSession{label: "live", recorder: recorder}
	service.poll = &orderedCloseSession{label: "poll", recorder: recorder}
	...
	workerAIndex := indexOf(order, "worker:session-1")
	workerBIndex := indexOf(order, "worker:session-2")
	if liveIndex < 0 || pollIndex < 0 || workerAIndex < 0 || workerBIndex < 0 {
		t.Fatalf("close order missing entries: %v", order)
	}
	if workerAIndex > liveIndex || workerAIndex > pollIndex || workerBIndex > liveIndex || workerBIndex > pollIndex {
		t.Fatalf("close order = %v, want all workers before live/poll", order)
	}
}

func TestMinimalLinkDiagnostics(t *testing.T) {
	service, _ := newMinimalService(t)
	logs := captureServiceLogs(service)

	service.logMinimalLinkWorkerLifecycle("minimal_link_worker_closed", lifecycleFields{
		"worker_generation":     uint64(42),
		"linked_thread":         "linked-t",
		"duration_ms":           int64(17),
		"rpc_code":              -32600,
		"session_identity_hash": "private-session-hash",
		"registry_key_hash":     "private-registry-hash",
		"prompt":                "PRIVATE_PROMPT_SENTINEL",
		"title":                 "PRIVATE_TITLE_SENTINEL",
		"transcript":            "PRIVATE_TRANSCRIPT_SENTINEL",
		"path":                  `C:\Users\private\state.sqlite`,
		"error":                 `secret=supersecret12345 C:\Users\private\state.sqlite`,
	})

	got := logs.String()
	for _, want := range []string{
		`"event":"minimal_link_worker_closed"`,
		`"generation":42`,
		`"linked_thread":"linked-t"`,
		`"duration_ms":17`,
		`"rpc_code":-32600`,
		...
		requireLogContains(t, got, want)
	}
	for _, forbidden := range []string{
		"session_identity", "registry_key", "PRIVATE_PROMPT_SENTINEL", "PRIVATE_TITLE_SENTINEL",
		"PRIVATE_TRANSCRIPT_SENTINEL", `C:\Users\private`, ".sqlite", "supersecret12345",
	} {
		if strings.Contains(got, forbidden) {
			t.Fatalf("minimal link diagnostic leaked %q in:\n%s", forbidden, got)
		}
	}
}
...
func TestMinimalAttachTrackedDoesNotResumeLegacyBoundPCThread(t *testing.T) {
	service, _ := newMinimalService(t)
	ctx := context.Background()
	project := service.cfg.Projects[0]
	thread := model.Thread{ID: "legacy-pc-bound-thread", CWD: project.CanonicalPath, Status: "completed"}
	if err := service.store.UpsertThread(ctx, thread); err != nil {
		t.Fatalf("UpsertThread failed: %v", err)
	}
	if err := service.store.SetBinding(ctx, 7, 0, thread.ID, model.BindingModeBound); err != nil {
		t.Fatalf("SetBinding failed: %v", err)
	}
	if err := service.store.SetState(ctx, minimalPCOriginThreadStatePrefix+thread.ID, "1"); err != nil {
		t.Fatalf("SetState(pc origin) failed: %v", err)
	}
	stub := &stubSession{}
	service.live = stub
	service.liveConnected = true
	...
	if len(stub.threadResumeCalls) != 0 {
		t.Fatalf("ThreadResume calls = %#v, want none for legacy PC binding", stub.threadResumeCalls)
	}
}

func TestMinimalPollTrackedSkipsNotLoadedUnknownOriginalWithoutResume(t *testing.T) {
	service, _ := newMinimalService(t)
	ctx := context.Background()
	project := service.cfg.Projects[0]
	thread := model.Thread{ID: "unknown-original-thread", CWD: project.CanonicalPath, Status: "not_loaded"}
	if err := service.store.UpsertThread(ctx, thread); err != nil {
		t.Fatalf("UpsertThread failed: %v", err)
	}
	if err := service.store.SetBinding(ctx, 7, 0, thread.ID, model.BindingModeBound); err != nil {
	...
	stub := &stubSession{threadReadErr: errors.New("thread not loaded")}
	service.poll = stub
	service.pollConnected = true
	...
	if len(stub.threadResumeCalls) != 0 {
		t.Fatalf("ThreadResume calls = %#v, want none for unknown original", stub.threadResumeCalls)
	}
}

func TestMinimalPollTrackedSkipsNotLoadedContinuationChildWithoutResume(t *testing.T) {
	service, _ := newMinimalService(t)
	ctx := context.Background()
	project := service.cfg.Projects[0]
	claim, created, err := service.store.ClaimMinimalContinuation(ctx, model.MinimalContinuation{
		Key:       model.MinimalContinuationKey{ChatID: 7, SourceThreadID: "pc-parent-thread", SourceTurnID: "turn-1"},
		ProjectID: project.ID,
	})
	if err != nil {
		t.Fatalf("ClaimMinimalContinuation failed: %v", err)
	}
	if !created {
		t.Fatal("ClaimMinimalContinuation did not create row")
	}
	child := model.Thread{ID: "continuation-child-thread", CWD: project.CanonicalPath, Status: "not_loaded"}
	if err := service.store.ActivateMinimalContinuation(ctx, *claim, child); err != nil {
		t.Fatalf("ActivateMinimalContinuation failed: %v", err)
	}
	if err := service.store.SetBinding(ctx, 7, 0, child.ID, model.BindingModeBound); err != nil {
		t.Fatalf("SetBinding failed: %v", err)
	}
	stub := &stubSession{threadReadErr: errors.New("thread not loaded")}
	service.poll = stub
	...
	if len(stub.threadResumeCalls) != 0 {
		t.Fatalf("ThreadResume calls = %#v, want none for continuation child", stub.threadResumeCalls)
	}
}
...
func TestMinimalPollOnlyApprovalSendsPCOnlyNoticeOnce(t *testing.T) {
	service, _ := newMinimalService(t)
	sender := &recordingSender{}
	service.SetSender(sender)
	ctx := context.Background()
	if err := service.store.SetGlobalObserverTarget(ctx, 7, 0, true); err != nil {
		t.Fatal(err)
	}
	project := service.cfg.Projects[0]
	thread := model.Thread{
		ID:           "poll-only-approval",
	...
	service.poll = &stubSession{threadReads: map[string]map[string]any{
		thread.ID: minimalWaitingApprovalPayload(thread, "PRIVATE_APPROVAL_BODY"),
	}}
	service.pollConnected = true
	...
	if len(sender.messages) != 1 {
		t.Fatalf("messages = %#v, want one PC-only approval notice", sender.messages)
	}
	notice := sender.messages[0]
	if len(notice.buttons) != 0 {
		t.Fatalf("notice buttons = %#v, want no remote approval buttons", notice.buttons)
	}
	for _, want := range []string{"승인", "PC"} {
		if !strings.Contains(notice.text, want) {
			t.Fatalf("notice text = %q, want %q", notice.text, want)
		}
	}
	for _, forbidden := range []string{"PRIVATE_APPROVAL_BODY", project.CanonicalPath, thread.Title} {
		if strings.Contains(notice.text, forbidden) {
			t.Fatalf("notice leaked %q: %q", forbidden, notice.text)
		}
	}
	...
	t.Cleanup(func() { _ = restarted.Close() })
	restarted.now = func() time.Time { return minimalTestNow.Add(time.Minute) }
	restartedSender := &recordingSender{}
	restarted.SetSender(restartedSender)
	restarted.poll = &stubSession{threadReads: map[string]map[string]any{
		thread.ID: minimalWaitingApprovalPayload(thread, "PRIVATE_APPROVAL_BODY"),
	}}
	restarted.pollConnected = true

	restarted.pollTracked(ctx)
	restarted.processDeliveryBatch(ctx)

	if len(restartedSender.messages) != 0 {
		t.Fatalf("restart replayed poll-only approval notice: %#v", restartedSender.messages)
	}
}

func minimalWaitingApprovalPayload(thread model.Thread, privateBody string) map[string]any {
...
func newNotifierServiceFromConstructor(t *testing.T) *Service {
	t.Helper()
	cfg := minimalStartupTestConfig(t)
	cfg.Profile = "notifier"
	cfg.Projects = nil
	cfg.ObserverPollInterval = time.Second
	cfg.DeliveryRetryBase = time.Millisecond
	cfg.DeliveryMaxAttempts = 3
	if err := cfg.Paths.Ensure(); err != nil {
		t.Fatal(err)
	}
	store, err := storage.OpenWithProtector(cfg.Paths.DBPath, securestore.NewDeterministicTestProtector())
...
type lifecycleCountingSession struct {
	stubSession
	mu     sync.Mutex
	starts int
	closes int
...
type blockingNotifierListSession struct {
	lifecycleCountingSession
...
type deadlineCapturingNotifierListSession struct {
	stubSession
	deadline time.Time
}

func (s *deadlineCapturingNotifierListSession) ThreadListWithOptions(ctx context.Context, options control.ThreadListOptions) (map[string]any, error) {
	s.deadline, _ = ctx.Deadline()
	return threadListPayload(), nil
}

func newBlockingNotifierListSession() *blockingNotifierListSession {
	return &blockingNotifierListSession{
...
func (s *blockingNotifierListSession) ThreadListWithOptions(ctx context.Context, options control.ThreadListOptions) (map[string]any, error) {
...
func (s *blockingNotifierListSession) Close() error {
	s.closeOnce.Do(func() { close(s.closeCalled) })
	return s.lifecycleCountingSession.Close()
}

func (s *blockingNotifierListSession) releaseList() {
	s.listReleaseOnce.Do(func() { close(s.listRelease) })
}

type blockingCloseContextSession struct {
	stubSession
	closeRelease     chan struct{}
	closeContextSeen chan struct{}
	closeContextOnce sync.Once
	releaseOnce      sync.Once
}

func newBlockingCloseContextSession() *blockingCloseContextSession {
	return &blockingCloseContextSession{
		closeRelease:     make(chan struct{}),
		closeContextSeen: make(chan struct{}),
	}
}

func (s *blockingCloseContextSession) Close() error {
	<-s.closeRelease
	return nil
}

func (s *blockingCloseContextSession) CloseContext(ctx context.Context) error {
	s.closeContextOnce.Do(func() { close(s.closeContextSeen) })
	select {
	case <-ctx.Done():
		return ctx.Err()
	case <-s.closeRelease:
...
func (s *blockingCloseContextSession) closeContextCalled() bool {
	select {
	case <-s.closeContextSeen:
		return true
	default:
		return false
	}
...
func (s *blockingCloseContextSession) release() {
	s.releaseOnce.Do(func() { close(s.closeRelease) })
}

type startSignalingSession struct {
	stubSession
	started   chan struct{}
	startOnce sync.Once
}

func newStartSignalingSession() *startSignalingSession {
	return &startSignalingSession{started: make(chan struct{})}
}

func (s *startSignalingSession) Start(context.Context) error {
	s.startOnce.Do(func() { close(s.started) })
	return nil
}

func (s *lifecycleCountingSession) Start(ctx context.Context) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.starts++
	return nil
...
func (s *lifecycleCountingSession) Close() error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.closes++
	return nil
...
func (s *lifecycleCountingSession) Starts() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.starts
}

func (s *lifecycleCountingSession) Closes() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.closes
}

func minimalStartupTestConfig(t *testing.T) config.Config {
	t.Helper()
	root := t.TempDir()
	projectPath := filepath.Join(root, "bridge")
	if err := ensureTestDir(projectPath); err != nil {
		t.Fatal(err)
	}
	return config.Config{
		...
		Projects: []model.Project{
			{ID: "bridge", DisplayName: "Bridge", CanonicalPath: projectPath},
		},
		Paths: config.Paths{
			Home:    root,
			DataDir: filepath.Join(root, "data"),
...
func seedLegacyMinimalContinuation(t *testing.T, service *Service, chatID, topicID int64, projectID, projectPath, sourceID, sourceTurnID, linkedID string) {
	t.Helper()
	ctx := context.Background()
	claim, created, err := service.store.ClaimMinimalContinuation(ctx, model.MinimalContinuation{
		Key: model.MinimalContinuationKey{
			ChatID: chatID, TopicID: topicID, SourceThreadID: sourceID, SourceTurnID: sourceTurnID,
		},
		ProjectID: projectID,
	})
	if err != nil || !created {
		t.Fatalf("ClaimMinimalContinuation(%s) created=%t err=%v", sourceID, created, err)
	}
	child := model.Thread{ID: linkedID, CWD: projectPath, Status: "completed"}
	if err := service.store.ActivateMinimalContinuation(ctx, *claim, child); err != nil {
		t.Fatal(err)
	}
}

func seedRunningMinimalLink(t *testing.T, service *Service, chatID, topicID int64, projectID, projectPath, sourceID, sourceTurnID, linkedID, activeTurnID string, generation uint64) {
	t.Helper()
	ctx := context.Background()
	source := model.Thread{ID: sourceID, CWD: projectPath, Status: "completed", Title: "Startup Source"}
	if err := service.store.UpsertThread(ctx, source); err != nil {
		t.Fatal(err)
	}
	link := model.MinimalLinkedThread{
		...
		SourceTitle:        "Startup Source",
		DesiredTitle:       "Startup Source · 텔레그램 연동",
	...
	child := model.Thread{ID: linkedID, CWD: projectPath, Status: "inProgress", ActiveTurnID: activeTurnID}
	if err := service.store.ActivateMinimalLinkedThread(ctx, link, provenance, child); err != nil {
		t.Fatal(err)
	}
	if changed, err := service.store.MarkMinimalLinkedTurnStarted(ctx, linkedID, generation, activeTurnID); err != nil || !changed {
		t.Fatalf("MarkMinimalLinkedTurnStarted(%s) changed=%t err=%v", linkedID, changed, err)
	}
}
...
type blockingBootstrapSession struct {
	stubSession
	started   chan struct{}
...
func newBlockingBootstrapSession() *blockingBootstrapSession {
	return &blockingBootstrapSession{
		started:   make(chan struct{}),
		releaseCh: make(chan struct{}),
	}
}

func (s *blockingBootstrapSession) ThreadList(ctx context.Context, limit int, cursor string) (map[string]any, error) {
	s.once.Do(func() {
		close(s.started)
	})
	<-s.releaseCh
...
func (s *blockingBootstrapSession) waitThreadListStarted(t *testing.T) {
	t.Helper()
	select {
	case <-s.started:
	case <-time.After(time.Second):
		t.Fatal("kickBootstrap ThreadList did not start")
	}
}

func (s *blockingBootstrapSession) release() {
	select {
	case <-s.releaseCh:
	default:
		close(s.releaseCh)
...
type contextBlockingBootstrapSession struct {
	stubSession
	started     chan struct{}
...
func newContextBlockingBootstrapSession() *contextBlockingBootstrapSession {
	return &contextBlockingBootstrapSession{
		started:   make(chan struct{}),
...
func (s *contextBlockingBootstrapSession) ThreadList(ctx context.Context, limit int, cursor string) (map[string]any, error) {
	s.startOnce.Do(func() {
		close(s.started)
	})
	select {
...
func (s *contextBlockingBootstrapSession) waitThreadListStarted(t *testing.T) {
	t.Helper()
	select {
	case <-s.started:
	case <-time.After(time.Second):
...
func (s *contextBlockingBootstrapSession) waitCanceled(t *testing.T) {
	t.Helper()
	select {
	case <-s.canceled:
	case <-time.After(time.Second):
		t.Fatal("kickBootstrap ThreadList was not canceled")
	}
}

func (s *contextBlockingBootstrapSession) release() {
...
func (s *stubSession) RespondServerRequestExact(ctx context.Context, requestID, threadID, turnID, requestKind string, result map[string]any) error {
	s.respondRequestCalls = append(s.respondRequestCalls, respondRequestCall{requestID: requestID, result: result})
	return nil
}
