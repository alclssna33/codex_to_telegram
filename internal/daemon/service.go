...
type Service struct {
	cfg             config.Config
	store           *storage.Store
	projectRegistry *projectregistry.Registry
	minimalRouter   *Router

	liveFactory          func() Session
	pollFactory          func() Session
	minimalWorkerFactory func() continuationSession
	minimalWorkers       *minimalLinkWorkerManager

	sessionMu                        sync.Mutex
	notifierCycleMu                  sync.Mutex
	mu                               sync.RWMutex
	live                             Session
	poll                             Session
	...
	wg                               sync.WaitGroup
	panelMu                          sync.Mutex
	minimalApprovalMu                sync.Mutex
	minimalApprovalSessionID         string
	voiceSessionIdentity             string
	...
	now                              func() time.Time
	afterMinimalPickerConsume        func()
	minimalLinkedCloseTimeout        time.Duration
	minimalLinkedFinalizationTimeout time.Duration
	started                          bool
	startedAt                        time.Time
	ready                            bool
	phase                            string
...
const (
	...
	minimalObserverCursorKey  = "minimal.observer.thread_list_cursor"
...
func New(cfg config.Config) (*Service, error) {
	...
	store, err := openStoreForProfile(cfg, runtime.GOOS)
	if err != nil {
		return nil, err
	}
...
func newWithStore(cfg config.Config, store *storage.Store) (*Service, error) {
	if store == nil {
		return nil, errors.New("storage store is required")
	}
	var projects *projectregistry.Registry
	var err error
	var notifierMigrationRetired int64
	if cfg.Profile == "minimal" {
		projects, err = projectregistry.New(cfg.Projects)
		if err != nil {
			return nil, fmt.Errorf("validate project registry: %w", err)
		}
		if err := store.EnsureMinimalSchema(context.Background()); err != nil {
			return nil, err
		}
		if _, err := store.RecoverCreatingMinimalContinuations(context.Background()); err != nil {
			return nil, err
		}
		if _, err := store.AdoptMinimalLinkedThreads(context.Background()); err != nil {
			return nil, err
		}
		if _, err := store.RecoverMinimalLinkedWorkers(context.Background()); err != nil {
			return nil, err
		}
		if _, err := store.RecoverClaimedPendingCommands(context.Background()); err != nil {
			return nil, err
		}
		if _, err := store.RecoverMinimalApprovalClaims(context.Background()); err != nil {
			return nil, err
		}
		if _, err := store.RecoverVoiceConfirmations(context.Background()); err != nil {
			return nil, err
		}
	} else if isNotifierProfile(cfg.Profile) {
		if err := store.EnsureNotifierSchema(context.Background()); err != nil {
			return nil, err
		}
		if _, err := store.EnsureNotifierActivation(context.Background(), time.Now().UTC()); err != nil {
			return nil, err
		}
		notifierMigrationRetired, err = store.MigrateNotifierProfile(context.Background(), time.Now().UTC())
		if err != nil {
			return nil, err
		}
	}
	...
	if cfg.Profile == "minimal" {
		service.minimalApprovalSessionID, err = newMinimalApprovalToken()
		if err != nil {
			return nil, fmt.Errorf("create minimal approval session identity: %w", err)
		}
		service.voiceSessionIdentity, err = newMinimalApprovalToken()
		if err != nil {
			return nil, fmt.Errorf("create voice session identity: %w", err)
		}
		if strings.TrimSpace(cfg.OpenAIAPIKey) != "" {
			service.voiceTranscriber = transcription.NewService(cfg.FFmpegBin, transcription.NewOpenAI(cfg.OpenAIAPIKey, cfg.TranscribeModel))
		}
	}
	...
	if !isNotifierProfile(cfg.Profile) {
		service.liveFactory = func() Session {
			return appserver.NewClient(cfg.CodexBin, cfg.AppServerListen, cfg.DefaultCWD, cfg.RequestTimeout)
		}
		service.minimalWorkerFactory = func() continuationSession {
			return appserver.NewClient(cfg.CodexBin, cfg.AppServerListen, cfg.DefaultCWD, cfg.RequestTimeout)
		}
		service.minimalWorkers = newMinimalLinkWorkerManager(service.minimalWorkerFactory, cfg.RequestTimeout, service.logMinimalLinkWorkerLifecycle)
		service.minimalWorkers.SetAcquireHook(service.startMinimalLinkedWorkerEventLoop)
		service.live = service.liveFactory()
	}
	service.poll = service.pollFactory()
	if cfg.Profile == "minimal" {
		service.minimalRouter = newMinimalRouter(service)
	}
	if isNotifierProfile(cfg.Profile) {
		service.logLifecycle("notifier_migration", lifecycleFields{"retired": notifierMigrationRetired})
	}
	return service, nil
}

func openStoreForProfile(cfg config.Config, goos string) (*storage.Store, error) {
	if !profileNeedsProtectedStore(cfg.Profile) {
		return storage.Open(cfg.Paths.DBPath)
	}
	if goos != "windows" {
		return nil, fmt.Errorf("%s profile requires Windows DPAPI; platform %s is unsupported", strings.TrimSpace(strings.ToLower(cfg.Profile)), goos)
	}
	return storage.OpenWithProtector(cfg.Paths.DBPath, securestore.NewDPAPIProtector())
}

func isNotifierProfile(profile string) bool {
	return strings.EqualFold(strings.TrimSpace(profile), "notifier")
}

func profileNeedsProtectedStore(profile string) bool {
	switch strings.TrimSpace(strings.ToLower(profile)) {
	case "minimal", "notifier":
		return true
	default:
		return false
	}
...
func (s *Service) logMinimalLinkWorkerLifecycle(event string, fields lifecycleFields) {
	safe := lifecycleFields{}
	if fields != nil {
		if value, ok := fields["worker_generation"]; ok {
			safe["generation"] = value
		} else if value, ok := fields["generation"]; ok {
			safe["generation"] = value
		}
		if value, ok := fields["linked_thread"]; ok {
			safe["linked_thread"] = shortLogID(fmt.Sprint(value))
		} else if value, ok := fields["linked_thread_id"]; ok {
			safe["linked_thread"] = shortLogID(fmt.Sprint(value))
		} else if value, ok := fields["thread_id"]; ok {
			safe["linked_thread"] = shortLogID(fmt.Sprint(value))
		}
		if value, ok := fields["source_thread"]; ok {
			safe["source_thread"] = shortLogID(fmt.Sprint(value))
		} else if value, ok := fields["source_thread_id"]; ok {
			safe["source_thread"] = shortLogID(fmt.Sprint(value))
		}
		if value, ok := fields["turn_id"]; ok {
			safe["turn_id"] = shortLogID(fmt.Sprint(value))
		} else if value, ok := fields["source_turn"]; ok {
			safe["source_turn"] = shortLogID(fmt.Sprint(value))
		}
		for _, key := range []string{"chat_hash", "project_id", "state", "duration_ms", "rpc_code"} {
			if value, ok := fields[key]; ok {
				safe[key] = value
			}
		}
		if value, ok := fields["error"]; ok {
			safe["error"] = sanitizeDiagnosticString(fmt.Sprint(value))
		}
	}
	s.logLifecycle(event, safe)
}
...
func (s *Service) Close() error {
	...
	var workerErr error
	if s.minimalWorkers != nil {
		workerErr = s.minimalWorkers.CloseAll(context.Background())
	}
	s.wg.Wait()
	s.sessionMu.Lock()
	s.mu.Lock()
	...
	return errors.Join(workerErr, s.store.Close())
}
...
func (s *Service) StatusSnapshot(ctx context.Context, chatID, topicID int64) (string, error) {
	if isNotifierProfile(s.cfg.Profile) {
		return s.notifierStatusSnapshot(ctx)
	}
	...
	lines = append(lines, "")
	if s.cfg.Profile == "minimal" && contextState.Binding != nil {
		if link, err := s.statusMinimalLinkedThread(ctx, chatID, topicID, contextState.Binding.ThreadID); err != nil {
			return "", err
		} else if link != nil {
			lines = append(lines, minimalLinkedStatusLines(*link, s.now())...)
			return strings.Join(lines, "\n"), nil
		}
		if continuation, err := s.store.ActiveMinimalContinuationForFork(ctx, chatID, topicID, contextState.Binding.ThreadID); err != nil {
			return "", err
		} else if continuation != nil {
			lines = append(lines,
				"Mode: Telegram continuation",
				fmt.Sprintf("Parent: %s", shortLogID(continuation.Key.SourceThreadID)),
				fmt.Sprintf("Fork: %s", shortLogID(continuation.ForkThreadID)),
				fmt.Sprintf("Anchor turn: %s", shortLogID(continuation.Key.SourceTurnID)),
			)
			return strings.Join(lines, "\n"), nil
		}
	}
...
func (s *Service) statusMinimalLinkedThread(ctx context.Context, chatID, topicID int64, boundThreadID string) (*model.MinimalLinkedThread, error) {
	boundThreadID = strings.TrimSpace(boundThreadID)
	if boundThreadID == "" {
		return nil, nil
	}
	if link, err := s.store.GetMinimalLinkedThreadByLinkedID(ctx, boundThreadID); err != nil {
		return nil, err
	} else if link != nil && link.ChatID == chatID && link.TopicID == topicID {
		return link, nil
	}
	return s.store.GetMinimalLinkedThread(ctx, chatID, topicID, boundThreadID)
}

func minimalLinkedStatusLines(link model.MinimalLinkedThread, now time.Time) []string {
	title := boundRunes(cleanMinimalDisplayText(link.DesiredTitle), 80)
	if title == "" {
		title = shortLogID(link.LinkedThreadID)
	}
	lines := []string{
		"Mode: Telegram linked task",
		"Task: " + title,
		fmt.Sprintf("Source: %s", shortLogID(link.SourceThreadID)),
		fmt.Sprintf("Linked: %s", shortLogID(link.LinkedThreadID)),
		"State: " + minimalLinkedStatusStateLabel(link.State),
	}
	if minimalLinkedBlockedRecently(link, now) {
		lines = append(lines, "Last acquire: blocked by another writer")
	}
	return lines
}

func minimalLinkedStatusStateLabel(state string) string {
	switch strings.TrimSpace(state) {
	case "", model.MinimalLinkedReady:
		return "bridge released"
	case model.MinimalLinkedTelegramRunning:
		return "Telegram worker"
	case model.MinimalLinkedReleasePending:
		return "release pending"
	case model.MinimalLinkedFailed:
		return "failed"
	default:
		return "unknown"
	}
...
func minimalLinkedBlockedRecently(link model.MinimalLinkedThread, now time.Time) bool {
	if !strings.HasPrefix(strings.TrimSpace(link.LastBlockedCode), "active_writer") || strings.TrimSpace(string(link.LastBlockedAt)) == "" {
		return false
	}
	blockedAt, err := time.Parse(time.RFC3339Nano, string(link.LastBlockedAt))
	if err != nil {
		return false
	}
	if now.IsZero() {
	...
	return now.UTC().Sub(blockedAt.UTC()) <= 24*time.Hour
}

func (s *Service) pendingMinimalLinkedTitleRetry(ctx context.Context, chatID, topicID int64) (bool, error) {
	if s == nil {
		return false, nil
	}
	contextState, err := s.store.GetChatContext(ctx, chatID, topicID)
	...
	if contextState.Binding == nil {
		return false, nil
	}
	link, err := s.statusMinimalLinkedThread(ctx, chatID, topicID, contextState.Binding.ThreadID)
	if err != nil || link == nil {
		return false, err
	}
	return strings.TrimSpace(link.TitleState) == model.MinimalLinkedTitlePending, nil
}

func (s *Service) repairMinimalLinkedReleasePending(ctx context.Context, chatID, topicID int64) (bool, error) {
	if s == nil || s.cfg.Profile != "minimal" || s.minimalWorkers == nil {
		return false, nil
	}
	contextState, err := s.store.GetChatContext(ctx, chatID, topicID)
	if err != nil || contextState.Binding == nil {
		return false, err
	}
	link, err := s.statusMinimalLinkedThread(ctx, chatID, topicID, contextState.Binding.ThreadID)
	if err != nil || link == nil {
	...
	turnID := strings.TrimSpace(link.ActiveTurnID)
	if generation == 0 || turnID == "" {
		return false, nil
	}
	sessionIdentity, ok := s.minimalWorkers.ConfirmedReleaseIdentity(link.LinkedThreadID, generation)
	if !ok {
		return false, nil
	}
	releaseCtx, cancel := s.minimalLinkedFinalizationContext()
	defer cancel()
	ready, err := s.minimalHandoffReadyDelivery(releaseCtx, *link, turnID)
	if err != nil {
		return false, err
	}
	changed, err := s.store.FinishMinimalLinkedReleaseWithReadyDelivery(releaseCtx, link.LinkedThreadID, generation, sessionIdentity, turnID, s.now(), ready)
	if err != nil || !changed {
		return changed, err
	}
	s.minimalWorkers.ForgetConfirmedRelease(link.LinkedThreadID, generation)
	s.logLifecycle("minimal_link_repair_finalized", lifecycleFields{
		"chat_hash":         shortTextHash(model.ChatKey(chatID, topicID)),
		"linked_thread":     shortLogID(link.LinkedThreadID),
...
func (s *Service) HandleMessage(ctx context.Context, chatID, topicID, userID int64, text string, replyToMessageID int64) (*DirectResponse, error) {
	if s.cfg.Profile == "minimal" {
		...
		if response, handled, err := s.handleMinimalMessageUserInputReply(ctx, chatID, topicID, text, replyToMessageID); handled || err != nil {
			return response, err
		}
		return s.HandleInboundText(ctx, model.InboundText{
			ChatID:           chatID,
			TopicID:          topicID,
			UserID:           userID,
			Text:             text,
...
func (s *Service) handleMinimalMessageUserInputReply(ctx context.Context, chatID, topicID int64, text string, replyToMessageID int64) (*DirectResponse, bool, error) {
	if replyToMessageID == 0 || minimalCommandName(text) != "" {
	...
	response, err := s.respondUserInputRequest(ctx, decision.RequestID, strings.TrimSpace(text))
...
func (s *Service) HandleCallback(ctx context.Context, chatID, topicID, messageID, userID int64, token string) (*DirectResponse, error) {
	if isNotifierProfile(s.cfg.Profile) {
		return s.handleNotifierCallback(ctx, userID, chatID)
	}
	if !s.IsAllowed(userID, chatID) {
		if s.cfg.Profile == "minimal" {
			return nil, ErrUnauthorized
	...
	if s.cfg.Profile == "minimal" {
		approvalRoute, approvalErr := s.store.GetMinimalApprovalRoute(ctx, token)
		if approvalErr != nil {
			return nil, approvalErr
		}
		if approvalRoute != nil {
			return s.handleMinimalApprovalCallback(ctx, chatID, topicID, messageID, approvalRoute)
		}
		voiceRoute, voiceErr := s.store.GetVoiceConfirmationRoute(ctx, token)
		if voiceErr != nil {
			return nil, voiceErr
		}
		if voiceRoute != nil {
			return s.handleMinimalVoiceCallback(ctx, chatID, topicID, messageID, voiceRoute)
		}
		pickerRoute, pickerErr := s.store.ConsumeMinimalPickerRoute(ctx, token, chatID, topicID, s.now())
		if pickerErr != nil {
			return nil, pickerErr
		}
		if pickerRoute != nil {
			if s.afterMinimalPickerConsume != nil {
				s.afterMinimalPickerConsume()
			}
			return s.handleMinimalPickerRoute(ctx, chatID, topicID, messageID, pickerRoute)
		}
	}
	...
	if s.cfg.Profile == "minimal" {
		...
		if route.Action != "answer_choice" {
			switch route.Action {
			case minimalProjectsAction, minimalProjectPickAction:
				return s.handleMinimalCallback(ctx, chatID, topicID, messageID, route)
			default:
...
func (s *Service) RegisterDirectDelivery(ctx context.Context, chatID, topicID, messageID int64, response *DirectResponse) error {
	...
	if response.VoiceConfirmationID != 0 {
		if err := s.store.BindVoiceConfirmation(ctx, response.VoiceConfirmationID, chatID, topicID, messageID); err != nil {
			_ = s.store.AbandonVoiceConfirmation(ctx, response.VoiceConfirmationID)
			return err
		}
	}
...
func (s *Service) ensureSessionLifecycle(ctx context.Context) {
	s.sessionMu.Lock()
	defer s.sessionMu.Unlock()
	s.ensureSessionLifecycleLocked(ctx)
}

func (s *Service) ensureSessionLifecycleLocked(ctx context.Context) {
	if isNotifierProfile(s.cfg.Profile) {
		s.ensurePollSessionLocked(ctx)
		return
	}
...
func (s *Service) handlePendingApprovalResolved(ctx context.Context, sessionIdentity string, event appserver.Event) bool {
	if !strings.EqualFold(strings.TrimSpace(event.Method), "serverRequest/resolved") {
		return false
	}
	...
	if sessionIdentity != "" {
		pending, err := s.store.GetPendingApprovalForSession(ctx, wireRequestID, sessionIdentity)
		if err != nil {
			return false
		}
		if pending != nil {
			_ = s.store.UpdatePendingApprovalStatusForSession(ctx, wireRequestID, sessionIdentity, "resolved")
			if pending.ThreadID != "" {
	...
	pending, err := s.store.GetPendingApproval(ctx, wireRequestID)
	if err != nil {
		return false
	}
	if pending != nil {
		if sessionIdentity != "" && pending.SessionIdentity != "" && pending.SessionIdentity != sessionIdentity {
			return false
		}
		_ = s.store.UpdatePendingApprovalStatus(ctx, pending.RequestID, "resolved")
		if s.cfg.Profile != "minimal" && pending.ThreadID != "" {
			s.syncThreadPanel(ctx, pending.ThreadID)
		}
	}
	return true
...
func (s *Service) handleLiveEvent(ctx context.Context, live Session, event control.Event) {
	if event.Channel == "transport_error" {
		err := fmt.Errorf("app-server transport error (%d params)", len(event.Params))
		_ = s.store.SetState(ctx, "appserver.live.last_error", sanitizeDiagnosticString(err.Error()))
	...
	if event.Channel == "transport_closed" {
		err := fmt.Errorf("app-server transport closed (%d params)", len(event.Params))
	...
	if s.handleMinimalApprovalServerRequest(ctx, live, s.minimalApprovalSessionIdentity(), event) {
		return
	}
	if s.handleMinimalApprovalResolved(ctx, s.minimalApprovalSessionIdentity(), event) {
		return
	}
	if s.handlePendingApprovalResolved(ctx, s.minimalApprovalSessionIdentity(), event) {
		return
	}
	if approval, ok := appserver.PendingApprovalFromServerRequest(event); ok {
		if s.cfg.Profile == "minimal" {
			approval.SessionIdentity = s.minimalApprovalSessionIdentity()
		}
		...
		if s.cfg.Profile != "minimal" && approval.ThreadID != "" {
...
func (s *Service) applyLiveToolSnapshot(ctx context.Context, threadID string, liveTool appserver.ThreadReadSnapshot) bool {
	...
	_ = s.persistThread(ctx, current.Thread)
	next := appserver.CompactSnapshot(state, s.snapshotForPersistence(current), time.Now().UTC())
...
func (s *Service) pollLoop(ctx context.Context) {
	...
	for {
		if isNotifierProfile(s.cfg.Profile) {
			s.runNotifierObservationCycle(ctx)
		} else {
...
// runNotifierObservationCycle keeps discovery, activation baselining, and
// bounded reads as one non-reentrant transaction at the daemon level.
func (s *Service) runNotifierObservationCycle(ctx context.Context) {
	if !s.notifierCycleMu.TryLock() {
		return
	}
	defer s.notifierCycleMu.Unlock()
	s.refreshObserverIndex(ctx)
	if ctx.Err() != nil {
		return
	}
...
func (s *Service) refreshObserverIndex(ctx context.Context) {
	if s.cfg.Profile == "notifier" {
		if err := s.discoverNotifierThreads(ctx); err != nil {
			if ctx.Err() == nil && errors.Is(err, context.DeadlineExceeded) {
				_ = s.RequestRepair(ctx, "notifier_thread_list_deadline")
			}
			s.noteSessionError(ctx, "notifier_thread_list", err)
		}
		return
	}
	...
	if s.cfg.Profile == "minimal" {
		if err := s.discoverMinimalRegisteredThreads(ctx); err != nil {
			s.noteSessionError(ctx, "minimal_thread_list", err)
		}
		return
	}
...
func (s *Service) repairSessions(ctx context.Context, reason string) {
	if isNotifierProfile(s.cfg.Profile) {
		s.repairNotifierPollSession(ctx, reason)
		return
	}
	s.sessionMu.Lock()
	s.mu.Lock()
...
func (s *Service) repairNotifierPollSession(ctx context.Context, reason string) {
	// Coordinate the session swap with an observation cycle so the outgoing
	// process cannot cancel an in-flight notifier list/read request.
	s.notifierCycleMu.Lock()
	s.sessionMu.Lock()
	s.mu.Lock()
	oldPoll := s.poll
	s.pollConnected = false
	if s.pollFactory != nil {
		s.poll = s.pollFactory()
	} else {
		s.poll = nil
	}
	...
	if oldPoll != nil {
		// A cancelled thread/read can leave the old process busy even after its
		// caller timed out. Retire it in the background so its shutdown does not
		// postpone a fresh notifier session and every other completion notice.
		s.spawn(ctx, func(closeCtx context.Context) {
			started := time.Now()
			err := s.closeNotifierPollSession(closeCtx, oldPoll)
			s.logAppServerCall("Close", started, err, oldPoll, lifecycleFields{"role": "poll", "operation": "repair_async"})
		})
	}
	_ = s.store.SetState(ctx, "appserver.poll_connected", "false")
	_ = s.store.SetState(ctx, "appserver.poll.generation", strconv.FormatUint(pollGeneration, 10))
	s.ensurePollSessionLocked(ctx)
	s.sessionMu.Unlock()
	s.notifierCycleMu.Unlock()
	s.bootstrapTrackedState(ctx)
}

func (s *Service) closeNotifierPollSession(ctx context.Context, poll Session) error {
	closer, ok := poll.(interface {
...
func (s *Service) discoverMinimalRegisteredThreads(ctx context.Context) error {
	...
	if pollConnected && poll != nil {
		client = poll
	} else if liveConnected && live != nil {
		client = live
	}
	if client == nil {
		return nil
	}
	cursor, err := s.store.GetState(ctx, minimalObserverCursorKey)
	if err != nil {
		return err
	}
	...
	started := time.Now()
	result, err := client.ThreadList(requestCtx, minimalThreadCatalogLimit, cursor)
	cancel()
	s.logAppServerCall("ThreadList", started, err, client, lifecycleFields{
		"operation": "minimal_registered_discovery",
		"limit":     minimalThreadCatalogLimit,
		"cursor":    cursor != "",
	})
	if err != nil {
		return err
	...
	for _, thread := range appserver.ThreadsFromList(result) {
		...
		project, ok := s.projectRegistry.MatchExactCWD(thread.CWD)
		if !ok {
			continue
		}
		...
		if err := s.store.ObserveMinimalThread(ctx, thread, project.ID, s.now()); err != nil {
			return err
		}
	}
	return s.store.SetState(ctx, minimalObserverCursorKey, minimalListNextCursor(result))
}
...
func (s *Service) pollTracked(ctx context.Context) {
	if s.cfg.Profile == "notifier" {
		if err := s.pollNotifierThreads(ctx); err != nil {
			s.noteSessionError(ctx, "notifier_thread_read", err)
		}
		return
	}
	s.mu.RLock()
	...
	for _, thread := range threads {
		...
		if err != nil {
			if isThreadNotLoadedError(err) {
				if s.cfg.Profile == "minimal" {
					s.logThreadReadSkipped(thread.ID, "thread_not_loaded_unowned")
					continue
				}
				requestCtx, cancel = context.WithTimeout(ctx, 5*time.Second)
				started = time.Now()
				_, resumeErr := poll.ThreadResume(requestCtx, thread.ID, thread.CWD)
				cancel()
				s.logAppServerCall("ThreadResume", started, resumeErr, poll, lifecycleFields{
					"operation": "poll_tracked_resume_not_loaded",
					"thread_id": thread.ID,
				})
				if resumeErr == nil {
					requestCtx, cancel = context.WithTimeout(ctx, maxDuration(10*time.Second, s.cfg.ObserverPollInterval*2))
					...
					cancel()
					s.logAppServerCall("ThreadRead", started, err, poll, lifecycleFields{
						"operation":      "poll_tracked_after_resume",
						"thread_id":      thread.ID,
						"include_turns":  true,
						"poll_catchup":   catchup,
						"poll_snapshot":  snapshot != nil,
		...
		current.Thread = mergeThreadMetadata(current.Thread, thread)
		retired, retireErr := s.retireMinimalObservationIfIneligible(ctx, current)
		if retireErr != nil {
			s.setError(ctx, retireErr)
			continue
		}
		if retired {
			continue
		...
		s.enqueueMinimalPollOnlyApprovalNotice(ctx, current, latestSnapshot)
		if s.applyTelegramOriginTerminalGate(ctx, "poll_tracked", &current, latestSnapshot) {
			continue
		}
		terminalLockHeld := false
		unlockTerminal := func() {}
		if s.cfg.Profile == "minimal" && s.minimalRouter != nil && isTerminalStatus(current.LatestTurnStatus) {
			unlockTerminal = s.minimalRouter.lockThread(current.Thread.ID)
			terminalLockHeld = true
			if stored, loadErr := s.store.GetThread(ctx, current.Thread.ID); loadErr == nil && stored != nil {
				if staleMinimalTerminalRefresh(*stored, current) {
					unlockTerminal()
					continue
				}
				current.Thread = mergeThreadMetadata(current.Thread, *stored)
			}
		}
		s.preserveTelegramOriginLiveCurrentTool(ctx, &current, latestSnapshot)
		_ = s.persistThread(ctx, current.Thread)
		nextSnapshot := appserver.CompactSnapshot(latestSnapshot, s.snapshotForPersistence(current), time.Now().UTC())
		if current.LatestTurnStatus == "inProgress" || current.WaitingOnApproval || current.WaitingOnReply {
		...
		s.handleTerminalSnapshot(ctx, current, terminalLockHeld)
		unlockTerminal()
		if s.cfg.Profile != "minimal" && (catchup || s.threadNeedsLiveSync(ctx, current.Thread.ID) || snapshotHasPassiveChange(latestSnapshot, &current)) {
...
func (s *Service) processDeliveryBatch(ctx context.Context) {
	...
	for _, item := range items {
		if item.Kind == minimalApprovalQueueKind {
			s.processMinimalApprovalDelivery(ctx, sender, item)
			continue
		}
		if item.Kind == minimalApprovalPCCleanupQueueKind {
			s.processMinimalApprovalPCCleanup(ctx, sender, item)
			continue
		}
		if item.Kind == minimalApprovalInactiveEditQueueKind {
			s.processMinimalApprovalInactiveEdit(ctx, sender, item)
			continue
		}
		...
		var messageID int64
		if payload.Rendered != nil {
			ids, sendErr := sender.SendRenderedMessages(ctx, item.ChatID, item.TopicID, []model.RenderedMessage{*payload.Rendered}, payload.Buttons, notifySendOptions())
			err = sendErr
			if len(ids) == 1 {
				messageID = ids[0]
			}
		...
		if err == nil && messageID <= 0 {
			err = errors.New("telegram delivery returned no message id")
		}
		if err != nil {
		...
		if payload.ThreadID != "" {
			completionErr = s.store.CompleteDeliveryWithRoute(ctx, item.ID, model.MessageRoute{
				ChatID:    item.ChatID,
			...
			completionErr = s.store.CompleteDelivery(ctx, item.ID)
		}
		if completionErr != nil {
			attempt := item.RetryCount + 1
			summary := deliveryErrorSummary("persist_error", completionErr)
...
func deliveryErrorSummary(status string, err error) string {
	if err == nil {
		return ""
	}
	switch status {
	case "decode_error", "send_error", "persist_error":
	default:
		status = "delivery_error"
	}
	message := err.Error()
	digest := sha256.Sum256([]byte(message))
	return fmt.Sprintf("class=telegram_delivery status=%s length=%d sha256=%x", status, len(message), digest[:6])
}
...
func (s *Service) handleCommand(ctx context.Context, chatID, topicID int64, raw string, replyToMessageID int64) (*DirectResponse, error) {
	...
	case "/repair":
		if s.cfg.Profile == "minimal" {
			if pendingTitle, err := s.pendingMinimalLinkedTitleRetry(ctx, chatID, topicID); err != nil {
				return nil, err
			} else if pendingTitle {
				s.logLifecycle("minimal_link_title_retry", lifecycleFields{
					"chat_hash": shortTextHash(model.ChatKey(chatID, topicID)),
				})
			}
			rearmed, err := s.store.RearmAmbiguousMinimalContinuations(ctx, chatID, topicID)
			if err != nil {
				return nil, err
			}
			if rearmed > 0 {
				s.logLifecycle("minimal_continuation_repair_rearmed", lifecycleFields{
					"chat_hash": shortTextHash(model.ChatKey(chatID, topicID)),
					"count":     rearmed,
				})
			}
			if _, err := s.repairMinimalLinkedReleasePending(ctx, chatID, topicID); err != nil {
				return nil, err
			}
		}
...
func (s *Service) ensureStartedTurnSnapshot(ctx context.Context, thread *model.Thread, turnID string) {
	...
	var minimalObservedProjectID string
	if s.cfg.Profile == "minimal" && s.projectRegistry != nil {
		if project, ok := s.projectRegistry.MatchExactCWD(startedThread.CWD); ok {
			startedThread.CWD = project.CanonicalPath
			startedThread.ProjectName = project.DisplayName
			minimalObservedProjectID = project.ID
		}
	}
	...
	nextSnapshot := appserver.CompactSnapshot(previous, s.snapshotForPersistence(current), time.Now().UTC())
	nextSnapshot.NextPollAfter = model.TimeString(time.Now().UTC().Add(s.cfg.ObserverPollInterval).Format(time.RFC3339Nano))
	_ = s.persistThread(ctx, startedThread)
	if minimalObservedProjectID != "" {
		_ = s.store.ObserveMinimalThread(ctx, startedThread, minimalObservedProjectID, s.now())
	}
...
func (s *Service) startTelegramOriginHotPoll(ctx context.Context, threadID, turnID string) {
	...
	go func() {
		defer s.wg.Done()
		s.telegramOriginHotPollLoop(runCtx, threadID, turnID)
	}()
}
...
func (s *Service) answerChoice(ctx context.Context, chatID, topicID int64, route *model.CallbackRoute) (*DirectResponse, error) {
	...
	if strings.TrimSpace(route.RequestID) == "" {
		if s.cfg.Profile == "minimal" {
			...
			return &DirectResponse{
				Text:         "이 선택지는 Codex에서만 답할 수 있습니다.",
				CallbackText: "Codex에서 답해주세요.",
	...
	response, err := s.respondUserInputRequestWithIdentity(ctx, route.RequestID, text, route.SessionIdentity)
	if err != nil {
		return nil, err
	}
	if response == nil {
...
func (s *Service) respondUserInputRequest(ctx context.Context, requestID, text string) (*DirectResponse, error) {
	return s.respondUserInputRequestWithIdentity(ctx, requestID, text, "")
}

func (s *Service) respondUserInputRequestWithIdentity(ctx context.Context, requestID, text, fallbackIdentity string) (*DirectResponse, error) {
	fallbackIdentity = strings.TrimSpace(fallbackIdentity)
	var approval *model.PendingApproval
	var err error
	if fallbackIdentity != "" {
		approval, err = s.store.GetPendingApprovalForSession(ctx, requestID, fallbackIdentity)
		if err != nil {
			return nil, err
		}
		if approval == nil && !minimalWorkerCallbackIdentity(fallbackIdentity) {
			approval, err = s.store.GetPendingApproval(ctx, requestID)
		}
	} else {
		approval, err = s.store.GetPendingApproval(ctx, requestID)
	...
	sessionIdentity := strings.TrimSpace(approval.SessionIdentity)
	if sessionIdentity == "" {
		sessionIdentity = fallbackIdentity
	}
	session, ok := s.sessionForCallbackIdentity(sessionIdentity, approval.ThreadID, approval.TurnID)
	if !ok {
		if sessionIdentity != "" && minimalWorkerCallbackIdentity(sessionIdentity) {
	...
	requestCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
	wireRequestID := firstNonEmpty(approval.WireRequestID, approval.RequestID)
	requestKind := firstNonEmpty(approval.RequestKind, approval.PromptKind)
	if err := exactResponder.RespondServerRequestExact(requestCtx, wireRequestID, approval.ThreadID, approval.TurnID, requestKind, userInputResponsePayload(approval.PayloadJSON, text)); err != nil {
		return &DirectResponse{Text: fmt.Sprintf("Could not send answer: %v", err)}, nil
	}
	_ = s.store.UpdatePendingApprovalStatus(ctx, approval.RequestID, "resolved:reply")
	s.syncThreadPanel(ctx, approval.ThreadID)
...
func (s *Service) trackedThreads(ctx context.Context, limit int) []model.Thread {
	...
	if s.cfg.Profile == "minimal" {
		due, _ := s.store.ListMinimalObservedThreadsDue(ctx, minimalObservedDueLimit)
	...
	for _, thread := range recent {
		...
		if s.cfg.Profile == "minimal" {
			observed, readRequired, err := s.store.MinimalObservationReadRequired(ctx, thread.ID)
			if err == nil && observed && !readRequired {
				continue
			}
		}
...
func (s *Service) refreshThreadForOperation(ctx context.Context, client Session, threadID, operation string) (*model.Thread, error) {
	...
	terminalLockHeld := false
	if s.cfg.Profile == "minimal" && s.minimalRouter != nil && isTerminalStatus(current.LatestTurnStatus) {
		unlockTerminal := s.minimalRouter.lockThread(threadID)
		defer unlockTerminal()
		terminalLockHeld = true
		if stored, loadErr := s.store.GetThread(ctx, threadID); loadErr != nil {
			return nil, loadErr
		} else if stored != nil {
			if staleMinimalTerminalRefresh(*stored, current) {
				return stored, nil
...
func (s *Service) persistThread(ctx context.Context, thread model.Thread) error {
	if s.cfg.Profile == "minimal" {
		var observedProjectID string
		if s.projectRegistry != nil && threadLooksActiveForPolling(thread) {
			if project, ok := s.projectRegistry.MatchExactCWD(thread.CWD); ok {
				thread.CWD = project.CanonicalPath
				thread.ProjectName = project.DisplayName
		...
		if observedProjectID != "" {
			_ = s.store.ObserveMinimalThread(ctx, thread, observedProjectID, s.now())
		}
		return nil
	}
	return s.store.UpsertThread(ctx, thread)
}

func (s *Service) snapshotForPersistence(snapshot appserver.ThreadReadSnapshot) appserver.ThreadReadSnapshot {
	if s.cfg.Profile != "minimal" {
		return snapshot
	}
	snapshot.Thread.Title = snapshot.Thread.ID
	snapshot.Thread.LastPreview = ""
	snapshot.Thread.Raw = nil
	snapshot.LatestAgentMessages = nil
	snapshot.LatestAgentMessageEntries = nil
	snapshot.LatestProgressText = ""
	snapshot.LatestUserMessageText = ""
	snapshot.LatestFinalText = ""
	snapshot.LatestToolLabel = ""
	snapshot.LatestToolOutput = ""
	snapshot.PlanPrompt = nil
	snapshot.DetailItems = nil
	return snapshot
}

func (s *Service) handleTerminalSnapshot(ctx context.Context, snapshot appserver.ThreadReadSnapshot, threadLockHeld bool) {
	if s.cfg.Profile == "minimal" {
		snapshot.LatestTurnStatus = canonicalMinimalTerminalStatus(snapshot.LatestTurnStatus)
	}
	if s.cfg.Profile != "minimal" || s.minimalRouter == nil || !isTerminalStatus(snapshot.LatestTurnStatus) {
		s.maybeLogTelegramOriginTerminal(ctx, snapshot)
		return
	}
	stale, staleErr := s.staleMinimalLinkedTerminal(ctx, snapshot)
	if staleErr != nil {
		s.setError(ctx, staleErr)
		return
	}
	if stale {
		return
	...
	s.maybeLogTelegramOriginTerminal(ctx, snapshot)
	var err error
	if projectErr := s.projectMinimalTerminal(ctx, snapshot); projectErr != nil {
		s.setError(ctx, projectErr)
		return
	}
	if releaseErr := s.releaseMinimalTerminalWorker(ctx, snapshot); releaseErr != nil {
		s.setError(ctx, releaseErr)
		return
	}
	drainCtx, drainCancel := s.minimalLinkedFinalizationContext()
	defer drainCancel()
	if threadLockHeld {
		err = s.minimalRouter.drainNextLocked(drainCtx, snapshot.Thread.ID)
	} else {
		err = s.minimalRouter.DrainNext(drainCtx, snapshot.Thread.ID)
	}
	if err != nil {
		s.setError(ctx, err)
	}
...
func staleMinimalTerminalRefresh(stored model.Thread, snapshot appserver.ThreadReadSnapshot) bool {
	if !threadLooksActiveForInput(&stored) || !isTerminalStatus(snapshot.LatestTurnStatus) {
		return false
	}
	activeTurnID := strings.TrimSpace(stored.ActiveTurnID)
	terminalTurnID := strings.TrimSpace(snapshot.LatestTurnID)
	return activeTurnID == "" || terminalTurnID == "" || activeTurnID != terminalTurnID
}
...
func (s *Service) callbackButton(ctx context.Context, text, action, threadID, turnID, requestID string, payload map[string]any) model.ButtonSpec {
	...
	if strings.TrimSpace(requestID) != "" {
		if pending, err := s.store.GetPendingApproval(ctx, requestID); err == nil && pending != nil {
			sessionIdentity = strings.TrimSpace(pending.SessionIdentity)
		}
	}
